...
from common import (
    REPO_ROOT, WIKI_DIR, RAW_DIR, INDEX_FILE, LOG_FILE,
    read_file, all_wiki_pages, resolve_wikilink, load_allowed_raw_dirs,
    sha256_full, ensure_utf8, init_env,
)
...
def check_index_sync(pages: list[Path]) -> dict:
    ...
    in_index_not_on_disk = [
        ...
        if str(p).startswith(str(WIKI_DIR))
    ]
...
def check_broken_links(pages: list[Path]) -> list[dict]:
    ...
    Delegates to common.resolve_wikilink() — the canonical resolver shared
    with lint.py — so both tools produce consistent results.
    Skips Raw/ provenance markers (intentional citations, not navigable links).
    """
    results = []
    for p in pages:
        content = read_file(p)
...
def check_hash_consistency() -> list[dict]:
    """Verify that Raw/ files recorded in log.md still match their stored sha256.

    Scans every log entry that contains a sha256: field, resolves the referenced
    Raw/ file, computes its current hash, and reports mismatches.

    A mismatch means the file was modified after ingestion — the researcher should
    confirm whether the corresponding Wiki pages need to be refreshed.

    Returns list of dicts with keys: log_line, raw_path, recorded_hash, current_hash.
    """
    log_content = read_file(LOG_FILE)
    if not log_content:
        return []
    ...
    def _sha256_of_file(path: Path) -> str:
        try:
            return sha256_full(path.read_bytes().decode("utf-8", errors="replace"))
        except OSError:
            return ""
    ...
    for line in log_content.splitlines():
        if "sha256:" not in line:
            continue
        # Extract sha256 hash
        m_hash = re.search(r'sha256:([0-9a-f]{8,64})', line)
        if not m_hash:
            continue
        recorded = m_hash.group(1)

        # Extract Raw/ file path from [[...]] wikilink
        m_path = re.search(r'\[\[(Raw/[^\]]+)\]\]', line)
        if not m_path:
            continue
        raw_rel = m_path.group(1).strip()
        raw_path = REPO_ROOT / raw_rel

        if not raw_path.exists():
            # File deleted or moved — report as missing rather than mismatch
            results.append({
                "log_line": line.strip()[:120],
                "raw_path": raw_rel,
        ...
        current_full = _sha256_of_file(raw_path)
        # Accept both short (8-char) and full (64-char) recorded hashes
        match = (recorded == current_full) or (recorded == current_full[:len(recorded)])
        if not match:
            results.append({
                "log_line": line.strip()[:120],
                "raw_path": raw_rel,
                "recorded_hash": recorded,
                "current_hash": current_full[:8] + "…",
                "status": "modified",
            })
...
def check_raw_compliance() -> list[str]:
    ...
    The whitelist is read from Tools/.raw_dirs (written by bootstrap when the
    user customises the Raw/ layout). Falls back to the default four directories
    (Sources / Thoughts / Records / Assets) if the file is absent.
...
def format_report(results: dict) -> str:
    ...
    # ── Hash Consistency
    hash_issues = results.get("hash_consistency", [])
    lines.append(f"## Hash Consistency ({len(hash_issues)} issue(s))")
    lines.append("")
    if hash_issues:
        lines.append("These Raw/ files were modified after ingestion. "
                     "Consider running `python Tools/refresh.py` to re-ingest changed files.")
        lines.append("")
        lines.append("| Raw File | Status | Recorded Hash | Current Hash |")
        lines.append("|---|---|---|---|")
        for hi in hash_issues:
            status_icon = "🔴 missing" if hi["status"] == "missing" else "🟡 modified"
            lines.append(
                f"| `{hi['raw_path']}` | {status_icon} "
                f"| `{hi['recorded_hash']}` | `{hi['current_hash']}` |"
            )
    else:
        lines.append("All ingested Raw/ files match their recorded hashes.")
    lines.append("")
