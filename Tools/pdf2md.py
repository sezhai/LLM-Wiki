#!/usr/bin/env python3
"""
Convert PDF or arXiv sources to Markdown for the Raw/Sources/ directory.

Usage:
    python tools/pdf2md.py <input> [--output Raw/Sources/output.md] [--backend auto]
...
Examples:
    ...
    python tools/pdf2md.py paper.pdf -o Raw/Sources/my-paper.md
"""
...
_RAW_DIRS_FILE = TOOLS_DIR / ".raw_dirs"


def _default_sources_dir() -> Path:
    """Return the first allowed Raw/ subdirectory that isn't Assets.

    Reads Tools/.raw_dirs if present (written by bootstrap for custom layouts),
    falls back to Raw/Sources/.  This keeps pdf2md aligned with whatever Raw/
    layout the user configured at bootstrap time.
    """
    if _RAW_DIRS_FILE.exists():
        names = [
            line.strip()
            for line in _RAW_DIRS_FILE.read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.strip().startswith("#")
            and line.strip().lower() != "assets"
        ]
        if names:
            return REPO_ROOT / "Raw" / names[0]
    return REPO_ROOT / "Raw" / "Sources"


DEFAULT_OUTPUT_DIR = _default_sources_dir()
...
def main():
    ...
    print(f"\nNote: file saved to Raw/Sources/ — ready for ingest.")
