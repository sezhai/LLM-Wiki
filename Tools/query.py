...
"""
    ...
    if save_path is not None:
        if save_path == "":
            ...
            # Normalise user-supplied path: ensure syntheses/ prefix and .md suffix
            sp = Path(save_path)
            if not sp.suffix:
                sp = sp.with_suffix(".md")
            parts = sp.parts
            if not parts or parts[0] != "syntheses":
                sp = Path("syntheses") / sp
            save_path = sp.as_posix()
...
"""
    ...
    # Append to log — distinguish saved syntheses from plain queries
    op_type = "query-synthesis" if save_path else "query"
    append_log(f"## [{today}] {op_type} | {question[:80]}\n\nSynthesized answer from {len(relevant_pages)} pages." +
