...
from common import (
    ...
    read_file, sha256_full as sha256, ensure_utf8, init_env,
...
def _parse_relation_annotations(content: str) -> dict[str, str]:
    """Extract semantic relation type annotations from the `related:` YAML field.

    Supports the optional inline comment syntax documented in AGENTS.md §3.1:
        related:
          - "[[Wiki/concepts/ConceptB.md]] # supports"
          - "[[Wiki/concepts/ConceptC.md]] # contradicts"

    Returns dict mapping target page_id (no .md) → relation string.
    Relation values: supports | contradicts | extends | depends_on | link (default).
    """
...
def build_extracted_edges(pages: list[Path]) -> list[dict]:
    ...
    Reads explicit [[wikilinks]] from page content and semantic relation type
    annotations from the `related:` YAML field (e.g. `# contradicts`).
    Edges include a `relation` field for graph-level analysis (e.g. contradicts density).
    """
    # Build maps for resolving wikilinks:
    #   stem_map: stem-only links like [[VPS运维与云服务器管理]]
    #   path_map: vault-absolute links like [[Wiki/concepts/VPS运维与云服务器管理]]
    ...
    for p in pages:
        content = read_file(p)
        src = page_id(p)
        # Pre-parse relation annotations for this page
        relation_annotations = _parse_relation_annotations(content)

        for link in extract_wikilinks(content):
            # Normalize vault-absolute paths (Wiki/ prefix)
            ...
            if target and target != src:
                ...
                if key not in seen:
                    seen.add(key)
                    # Look up semantic relation from annotations (fallback: "link")
...
def build_inferred_edges(pages: list[Path], existing_edges: list[dict], cache: dict, resume: bool = True) -> list[dict]:
    ...
    # Skip if no LLM model is configured (no API key / local model available)
    if not os.getenv("LLM_MODEL_FAST"):
        print("  skipped: LLM_MODEL_FAST not set (set env var or use --no-infer to skip)")
        return []
...
"""
        ...
        valid_rels = []
        try:
            raw = call_llm(prompt, "LLM_MODEL_FAST", None, max_tokens=1024)
            raw = raw.strip()
...
def find_phantom_hubs(pages: list[Path], min_refs: int = 2) -> list[dict]:
    ...
    for p in pages:
        ...
        for link in links:
            ...
            if normalized.startswith('wiki/'):
                normalized = normalized[5:]
            # Skip Raw/ provenance markers — they are intentional references to source notes
            if normalized.startswith('raw/'):
                continue
...
def generate_report(nodes: list[dict], edges: list[dict], communities: dict[str, int],
    ...
    # Contradicts density section (C5 fix: uses relation field from extracted edges)
    ...
    if n_contradicts == 0:
        lines.append(
            "No `contradicts` edges found. Annotate conflicting concepts in `related:` "
            "fields with `# contradicts` to surface systematic disagreements."
        )
    else:
        density_label = (
            "🔴 high — systematic disagreements present" if contradicts_density > 20
            else "🟡 moderate" if contradicts_density > 8
            else "🟢 low"
        )
        lines.append(
            f"{n_contradicts} contradicts edge(s) out of {n_edges} total "
            f"({contradicts_density:.1f}%) — {density_label}"
        )
        lines.append("")
...
def build_graph(infer: bool = True, open_browser: bool = False, clean: bool = False,
    ...
    # Save graph.json — Bootstrap §九 schema: extracted and inferred edges in separate keys
    ...
    if report:
        if not HAS_NETWORKX:
            ...
            append_log(f"## [{today}] graph | Graph health report generated\n\n{len(nodes)} nodes analyzed.")

    if open_browser:
        import urllib.parse
        webbrowser.open(urllib.parse.urlunparse(("file", "", str(GRAPH_HTML.resolve()), "", "", "")))
