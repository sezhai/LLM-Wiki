...
│   ├── raw-mapping.json       # Raw 子目录→加注类型映射
...
└── Tools/                     # 工具脚本（bootstrap 时全部写入）
    ...
    ├── common.py              # 共享依赖库
    ├── health.py              # 健康检查脚本
    ├── ingest.py              # 摄入脚本（支持 --standalone 并持久化到标记文件）
    ├── lint.py                # 质量审计脚本
    ├── query.py               # 查询脚本
...
- [ ] 2. 验证并清理目标目录
- [ ] 3. 创建物理目录结构
- [ ] 4. 写入 `AGENTS.md`（含立即写入 `.llm-wiki/raw-mapping.json` 和 `.llm-wiki/domain-enum.json`）
- [ ] 5. 写入 `README.md`
- [ ] 6. 写入 `.gitignore`
- [ ] 7. 写入 `Tools/requirements.txt` 和 `.env.example`，并安装依赖
- [ ] 8. 写入 `Wiki/` 初始核心文件
- [ ] 9. 写入所有 `Tools/` 脚本
- [ ] 10. 初始化 Git 仓库（含用户配置兜底）
- [ ] 11. 追加 bootstrap 记录到 `log.md`
- [ ] 12. 首次 Git 提交（含完整 `git add`）
- [ ] 13. 最终验证与报告（含 qmd 集合初始化提示）

### 1.1 向用户询问关键配置

**在写入任何文件之前**，通过一次对话确认以下四项。若用户拒绝回答或说"用默认"，使用括号内的默认值：

1. **Raw 层目录**：使用默认四类目录（`Sources / Thoughts / Records / Assets`），还是自定义？（默认：使用四类默认目录。若自定义，请用户列出目录名及各自的加注类型和是否参与摄入，后续所有涉及 Raw 子目录的规则均以用户提供的映射为准，包括 `raw-mapping.json`、`AGENTS.md` 模板；自定义时将不使用任何默认目录）

2. **Git 远程**：是否配置 `origin`？若是，提供 URL。（默认：暂不配置）

3. **摄入风格偏好**（写入 AGENTS.md §四 `默认摄入风格` 处）：
   - **批量模式（默认）**：说 `ingest` 时 Agent 先运行 `--scan` 获取队列，再逐文件调用 `python -m Tools.ingest <路径>`，整批完成后汇报新建词条
   - **单篇讨论模式**：说 `ingest` 时 Agent 每次只处理一个文件，摄入后与研究者讨论关键要点，确认后再继续下一个

   用户选择后，将对应模式名称写入 AGENTS.md §四首部的 `默认摄入风格：` 行。**注意**：无论用户选择哪种默认风格，另一种模式始终可通过显式参数临时切换。

4. **知识领域（domain）枚举**：使用顿号分隔的列表，默认 `哲学、历史、技术、经济学、文学、自然科学、宗教、其他`。是否增删？（默认：使用以上枚举。用户可增补如 `金融、法学` 等。Agent 将据此生成 `.llm-wiki/domain-enum.json`，并将枚举列表以顿号分隔形式写入 AGENTS.md 的 `domain` 字段说明中，**同时将枚举中的第一项写入 §3.1 frontmatter 示例的 `domain` 字段注释里**，确保示例值始终来自用户实际确认的枚举。）
...
- `Wiki/` 下各子目录**绝对禁止**再建子文件夹，但必须将以下五个子目录全部创建：  
  `concepts/`, `entities/`, `sources/`, `syntheses/`, `disambiguations/`
...
- `Tools/` 立即创建空文件 `Tools/__init__.py`（执行 `touch Tools/__init__.py`）（使 Tools/ 成为 Python 包）
...
| `<raw-dir-table>` | 用户在 §1.1 确认的 Raw 子目录映射表（**含完整 Markdown 表格**，默认如下）；替换时直接将占位符行替换为表格内容 |
| `<DEFAULT_INGEST_MODE>` | 用户在 §1.1 选择的摄入风格名称，例如：`批量模式` 或 `单篇讨论模式` |
| `<DOMAIN_ENUM>` | 用户在 §1.1 确认的 domain 枚举列表，格式为顿号分隔的中文词（如 `哲学、历史、技术、经济学、文学、自然科学、宗教、其他`）。此占位符出现在 AGENTS.md 的说明段落中，表示"全部合法领域"。 |
| `<DOMAIN_FIRST>` | 用户在 §1.1 确认的枚举列表中的**第一项**（如枚举为 `哲学、历史、...`，则替换为 `哲学`；若为 `金融、法学、...`，则替换为 `金融`）。此占位符用于 §3.1 frontmatter 示例的 `domain` 字段注释，确保示例值始终属于用户实际确认的枚举。 |
...
|------------|----------|---------------|----------|
| `Sources/` | 他人文档：论文、网页剪藏、技术规范、书籍 | `（据文献）` | 引文保真；他人观点与推论一律加注 |
...
Agent 根据 §1.1 第4项确认的枚举列表生成如下 JSON（默认）：

```json
{
  "version": 1,
  "domains": ["哲学","历史","技术","经济学","文学","自然科学","宗教","其他"]
}
```
...
- **Raw 层目录说明**（按 §1.1 确认的目录列出）
- **Wiki 层用户可见分类**（sources / syntheses 标注为自动生成，研究者无需手动管理）
- **三步上手**：
  1. 把文件放进 `Raw/` 对应子目录
  2. 说 `ingest` 触发摄入（默认批量模式，Agent 先扫描再逐个处理；说 `ingest --discuss` 切换为单篇讨论模式；若想临时处理某个文件而不影响批次进度，使用 `ingest --standalone <文件>`）
  3. 说 `query: <问题>` 触发查询
- **qmd 集合初始化提示**：若安装了 qmd，需分别为 Wiki 层和 Raw 层初始化集合：`qmd init --collection wiki Wiki/` 和 `qmd init --collection raw Raw/`（健康检查会自动提示缺失）
...
> 固定次版本范围（`<0.2.0`）以确保 `markitdown <file>` CLI 调用方式的稳定性。升级前请验证 CLI 接口兼容性。
...
# ────────────────────────────────────────────────────────
# LLM Wiki 环境变量配置
# 默认行为：所有需要 LLM 的操作（图谱推断、加注检查等）
# 自动委托给当前 Agent（opencode/claude/cursor 等），无需配置任何变量。
# 以下变量仅在需要使用独立外部 LLM CLI 时配置。
# ────────────────────────────────────────────────────────

# 外部 LLM CLI 路径（可选）
# 配置后，所有 agent_llm() 调用将通过该 CLI 执行，而非委托给当前 Agent
# 要求：该 CLI 接受单个 prompt 字符串参数，将结果输出到 stdout
# 示例：LLM_BACKEND=/usr/local/bin/llm  （Simon Willison 的 llm 工具）
# LLM_BACKEND=

# qmd 搜索引擎路径（可选，若未在 PATH 中则指定完整路径）
# QMD_BIN=/usr/local/bin/qmd

# 本地或兼容 OpenAI 格式的模型服务端点（可选，供 LLM_BACKEND 使用）
...
**`Wiki/overview.md`**（含结构模板，供 Agent 首次更新时参照）：
...
> 本文件由 Agent 维护。触发条件：单次摄入涉及 2+ 个不同 `domain` 的词条，
> 或现有判断被新料直接否定时更新。Agent 须保持本文件有意义，不得写入占位文字。

## 当前研究焦点

<!-- 用 2–4 句话描述知识库目前积累最多的主题领域 -->
...
<!-- 操作类型：bootstrap / ingest / query-synthesis / health / lint / graph / chore / ERROR -->
...
<!-- health 完整格式：## [YYYY-MM-DD] health | <通过/N项警告> -->
<!-- lint 完整格式：## [YYYY-MM-DD] lint | <通过/N项严重问题> -->
<!-- chore move 格式：## [YYYY-MM-DD] chore | move: [[Raw/<旧路径>]] → [[Raw/<新路径>]] -->
<!-- chore delete 格式：## [YYYY-MM-DD] chore | delete: [[Raw/<子目录>/<文件名>]] -->
<!-- chore rename 格式：## [YYYY-MM-DD] chore | rename: [[Wiki/<旧路径>]] → [[Wiki/<新路径>]] | slug:<新文件名去后缀> -->
```
...
def _check_markitdown() -> bool:
    try:
        _sp.run(["markitdown", "--version"], capture_output=True, timeout=5, check=True)
        return True
    except Exception:
        return False

MARKITDOWN_AVAILABLE: bool = _check_markitdown()
...
def all_wiki_pages() -> List[Path]:
    ...
    if not WIKI_DIR.exists():
        return []
    excluded_names = {"health-report.md", "lint-report.md", "_semantic_lint_context.md"}
    return sorted(
...
def load_allowed_raw_dirs() -> List[Dict]:
    ...
    条目格式：{"name": str, "annotation": str|None, "ingest": bool}
    配置文件缺失时抛出明确错误。
    """
...
def check_slug_conflict(
    ...
    """
    检查 slug 是否已存在于磁盘文件或 log.md 记录中。
    返回 (conflict: bool, existing_source: str | None)。
    """
    target_path = WIKI_DIR / target_dir.strip("/") / f"{slug}.md"
    if target_path.exists():
        return True, target_path.relative_to(REPO_ROOT).as_posix()

    if LOG_FILE.exists():
        log_text = LOG_FILE.read_text(encoding="utf-8")
        pattern = rf"## \[\d{{4}}-\d{{2}}-\d{{2}}\] {re.escape(operation_type)}.*slug:{re.escape(slug)}(\s|\||\(|$)"
        m = re.search(pattern, log_text)
        if m:
            return True, f"log.md:{m.group(0)[:80]}"

    return False, None


def parse_log_slugs(log_text: str) -> Set[str]:
    """从 log.md 解析所有已记录的 slug 集合（与操作类型无关）。"""
    slug_pattern = re.compile(r"(?:^|\s|\|)\s*slug:([^\s|]+)", re.MULTILINE)
    return set(m.group(1).strip() for m in slug_pattern.finditer(log_text))

# ─────────────────────────────────────────────
# 词条 pending_review 扫描
# ─────────────────────────────────────────────
...
def find_pages_by_raw_source(raw_rel: str) -> List[Path]:
    """
    返回所有 Wiki 词条（任何子目录），其 frontmatter 中 sources/raw_link 字段
    包含指定 Raw 文件的引用。
    raw_rel：相对于 REPO_ROOT 的路径字符串，如 "Raw/Sources/某论文.md"
    """
    result = []
    ...
    for page in all_wiki_pages():
        ...
        links = extract_wikilinks(fm_text)
        normalized_links = {normalize_path_str(l) for l in links}
        if target in normalized_links:
            result.append(page)
            continue

        str_refs = re.findall(r'"?\[\[([^\]]+)\]\]"?', fm_text)
        str_normalized = {normalize_path_str(r.split("|")[0].strip()) for r in str_refs}
        if target in str_normalized:
            result.append(page)
...
def parse_moved_paths(log_text: str) -> Dict[str, str]:
    """
    解析 log.md 中所有 chore | move: 记录，返回 {原始路径: 最新路径} 映射。
    支持多步链式 move（A→B→C 最终映射 A→C）。
    """
    ...
    for m in move_pattern.finditer(log_text):
        ...
        origin = reverse.get(old_path, old_path)
        moves[origin] = new_path
        reverse[new_path] = origin
...
def load_graph_json() -> Optional[dict]:
    """加载并解析 Graph/graph.json，文件不存在或损坏时返回 None。"""
...
def agent_llm(prompt: str, output_var: str = "LLM_OUTPUT") -> str:
    """
    向当前 Agent 发出 LLM 推理请求。

    工作机制：
    - 优先检查 LLM_BACKEND 环境变量（用户配置了独立 CLI 时走外部调用）
    - LLM_BACKEND_MODE=stdin 时，通过 stdin 传递 prompt
    - LLM_BACKEND_MODE=arg 时，通过位置参数传递 prompt（默认为 stdin）
    - 未配置时：向 stdout 打印结构化 [AGENT_LLM_REQUEST] 块，
      由当前 Agent 读取并在下一步中作为上下文执行推理
    """
    backend = os.environ.get("LLM_BACKEND", "").strip()
    backend_mode = os.environ.get("LLM_BACKEND_MODE", "stdin").strip().lower()
    request_id = str(uuid.uuid4())[:8]

    if backend:
        try:
            if backend_mode == "stdin":
                result = _sp.run(
                    [backend],
                    input=prompt,
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                print(f"⚠️ LLM_BACKEND 调用失败（exit {result.returncode}），回退至当前 Agent 模式")
            else:
                print("⚠️ LLM_BACKEND 返回空输出，回退至当前 Agent 模式")
        except Exception as e:
            print(f"⚠️ LLM_BACKEND 异常: {e}，回退至当前 Agent 模式")
    ...
    print(f"请根据以上提示完成推理，将结果作为 {output_var} 使用。\n")
...
from Tools.common import (
    ...
    sha256_file, list_pending_review,
    git_add_commit, parse_log_slugs, parse_moved_paths, QMD_AVAILABLE,
)
...
def check_index_sync(pages: Dict[str, Path]) -> List[str]:
    ...
    for lnk in linked:
        p = WIKI_DIR / lnk
        if p.exists():
            index_paths.add(p.relative_to(REPO_ROOT).as_posix())
...
def check_log_coverage(pages: Dict[str, Path]) -> List[str]:
    """检查有词条页面但 log.md 中缺少对应记录（syntheses 若未由完整查询工作流生成会被报告）。"""
    issues = []
    if not LOG_FILE.exists():
        return ["log.md 不存在"]
    log_text = read_file(LOG_FILE)
    all_slugs = parse_log_slugs(log_text)
    ...
    for rel, p in pages.items():
        ...
        if stem in excluded_names:
            continue
        if stem not in all_slugs:
            issues.append(f"日志覆盖缺失: {rel}")
    return issues


def check_overview_placeholder() -> List[str]:
    """检查 overview.md 中所有 section 是否仍为模板占位注释，逐一报告空 section。"""
    issues = []
    ...
    for sec in sections:
        ...
        body = sec[len(header_match.group(0)):]
        # 移除 HTML 注释
        body_clean = re.sub(r"<!--.*?-->", "", body, flags=re.DOTALL).strip()
        if not body_clean:
            issues.append(f"overview.md 的"{title}"节仍为占位内容，请 Agent 更新")
    return issues
...
def check_hash_consistency() -> List[str]:
    ...
    算法与 Tools.ingest._load_log_index 相同（物理顺序覆盖，后者为准）。
    """
    issues = []
    if not LOG_FILE.exists():
        return []
    ...
    # 物理顺序覆盖，与 _load_log_index 一致
    ingest_by_path: Dict[str, str] = {}
    ingest_pattern = re.compile(
        r"^## \[\d{4}-\d{2}-\d{2}\] ingest \| .+ \| \[\[(.+?)\]\] \| sha256:([0-9a-f]{32})",
        re.MULTILINE
    )
    for m in ingest_pattern.finditer(log_text):
        raw_rel, recorded_hash = m.group(1), m.group(2)
    ...
    for raw_rel, recorded_hash in ingest_by_path.items():
        ...
        if not file_path.exists():
            if effective_path != raw_rel:
                issues.append(
                    f"⚠️ [move 后路径不存在] 原路径: {raw_rel} → 当前路径: {effective_path}\n"
                    f"   文件可能已再次移动或删除，请执行 chore: move 或 chore: delete 更新记录"
                )
            else:
                issues.append(
                    f"⚠️ [路径不存在] {raw_rel}：文件可能已被移动或删除，"
            ...
            if current_hash != recorded_hash:
                issues.append(
                    f"⚠️ [内容已修改] {effective_path}：当前哈希与摄入记录不一致，"
...
def run_health(save: bool = False, as_json: bool = False) -> int:
    ...
    if as_json:
        ...
        for category, issues in results.items():
            if "日志覆盖" in category or "Overview占位" in category:
                severity = "💡"
                level = "（质量建议，非阻塞）"
            else:
                severity = "⚠️"
                level = "（结构错误，需修复）"
            status = "✅ 通过" if not issues else f"{severity}  {len(issues)} 项{level}"
            print(f"\n[{category}] {status}")
            for issue in issues:
                print(f"  - {issue}")
        print("\n" + "=" * 60)
        if passed:
            ...
            print(f"⚠️  共发现 {len(all_issues)} 项问题，请处理后重新检查。")
        print("=" * 60)
...
def _load_log_index() -> Dict[str, str]:
    ...
    for m in pattern.finditer(read_file(LOG_FILE)):
        result[m.group(1)] = m.group(2)
    return {normalize_path_str(k): v for k, v in result.items()}
...
def scan_pending() -> None:
    if INGEST_QUEUE_FILE.exists():
        try:
            old_queue = json.loads(INGEST_QUEUE_FILE.read_text(encoding="utf-8"))
            if old_queue.get("pending"):
                print(f"⚠️ 当前有未完成批次（{len(old_queue['pending'])} 个待处理），重新扫描将覆盖进度。")
                print("   若要继续，请先删除 .llm-wiki/ingest-queue.json 或完成当前批次。")
                return
        except Exception:
            pass
    ...
    write_file(INGEST_QUEUE_FILE, json.dumps(queue_data, ensure_ascii=False, indent=2))

    print(f"📋 发现 {len(pending)} 个待摄入/已修改文件，已写入批次清单：")
    for i, p in enumerate(pending, 1):
        print(f"  {i}. {p}")
    print(
        f"\n💡 请依次运行: python -m Tools.ingest <文件路径>\n"
        f"   每个文件 --finalize 后将自动从批次清单中勾除，并报告剩余数量。"
    )
...
def _convert_to_markdown(file_path: Path) -> Tuple[bool, str]:
    ...
    if not MARKITDOWN_AVAILABLE:
        print(
            f"⚠️ markitdown 未安装，无法转换 {file_path.name}。\n"
    ...
    try:
        ...
        print(f"⚠️ markitdown 转换失败: {file_path.name}\n{result.stderr[:200]}")
        return False, ""
    except Exception as e:
        print(f"⚠️ markitdown 转换异常: {file_path.name}: {e}")
        return False, ""
...
def _update_index(slug: str, subdir: str, title: str, brief: str = "") -> None:
    ...
    if next_section == -1:
        ...
        if content[:next_section].endswith("\n"):
            content = content[:next_section] + link_line + content[next_section:]
        else:
            content = content[:next_section] + "\n" + link_line + content[next_section:]
...
# 日志写入
# ─────────────────────────────────────────────
def _write_log_entry(title: str, raw_rel: str, file_hash: str, slug: str) -> None:
    today = date.today().isoformat()
    entry = (
...
def ingest_file(
    ...
    if standalone:
        try:
            file_hash_full = sha256_file(file_path, truncate=None)
            path_hash = hashlib.sha256(str(file_path).encode()).hexdigest()[:8]
            flag_dir = REPO_ROOT / ".llm-wiki"
            flag_dir.mkdir(parents=True, exist_ok=True)
            flag_file = flag_dir / f".standalone-{path_hash}-{file_hash_full[:16]}.flag"
            flag_file.touch()
        except Exception as e:
            print(f"⚠️ 无法创建 standalone 标记文件: {e}。请在 --finalize 时务必显式传递 --standalone 参数。")
    ...
    if raw_size > MAX_BYTES and not force:
        print(
            f"\n⚠️ 大文件警告：{rel}（{raw_size:,} 字节，约 {raw_size // 1000} KB）。\n"
    ...
    if raw_size > MAX_BYTES and force:
        print(f"⚠️ 注意：文件超大（{raw_size:,} 字节），已通过 --force 强制摄入，请注意上下文窗口限制。")
    ...
    if rel in log_index and sha256_file(file_path) != log_index[rel]:
        ...
        for page in related_pages:
            content = read_file(page)
            if re.search(r"^pending_review:\s*false\s*$", content, re.MULTILINE):
                updated = re.sub(
                ...
                print(f"   ℹ️ 已标记 {page.name} 为 pending_review（源文件已修改）")
    ...
    print(f"双链格式：所有内部链接使用基于 Vault 根目录的绝对路径，如 [[Wiki/concepts/概念名.md]]")
    print(f"Wiki 索引: {'存在' if index_exists else '不存在'} | 综述: {'存在' if overview_exists else '不存在'}")
    print(f"\n--- 文件内容（前 3000 字符）---\n{md_content[:3000]}\n---")

    finalize_flags = " --standalone" if standalone else ""
    if discuss_mode:
        print(
            f"\n[DISCUSS] {file_path.stem}\n"
            f"请与研究者讨论关键要点，确认后：\n"
            f"1. 写入词条\n"
            f"2. 执行：python -m Tools.ingest --finalize <slug> <title> {rel} --brief \"<一句话摘要>\"{finalize_flags}\n"
        )
        return 2

    print(
        f"\n请 Agent 根据以上内容：\n"
        f"1. 写入/更新 Wiki/sources/ 下的摘要词条（文件名：中文标题.md）\n"
        f"2. 更新 Wiki/concepts/ 和 Wiki/entities/ 下的相关词条\n"
        f"3. 检查所有新词条中 [[双链]] 是否指向已存在页面（断链需同步新建桩页）\n"
        f"4. 确定 slug（词条中文文件名去后缀）、title（显示标题）、brief（一句话摘要，≤30字）\n"
        f"5. 完成后执行：python -m Tools.ingest --finalize <slug> <title> {rel} --brief \"<brief>\"{finalize_flags}\n"
    )
    return 0
...
def _remove_from_queue(rel: str, skip: bool = False) -> None:
    if not INGEST_QUEUE_FILE.exists():
        return
    try:
        queue_data = json.loads(read_file(INGEST_QUEUE_FILE))
...
def _update_batch_progress(raw_rel: str, standalone: bool) -> None:
    """更新批次清单并输出停止/继续信号。"""
    if standalone:
        print("ℹ️ standalone 模式，不更新批次进度。")
        print("[STOP] 本次为独立摄入，批次不受影响。")
        return

    if not INGEST_QUEUE_FILE.exists():
        print("[STOP] 无批次清单，本次为独立摄入，批次不受影响。")
        return
    ...
    try:
        ...
        if raw_rel in done:
            print(f"\nℹ️ {raw_rel} 已在本批次中处理完成，跳过重复记录。")
            print("[STOP] 该文件已处理完成，无需重复操作。")
            return
        ...
        if raw_rel in pending:
            ...
            if remaining == 0:
                print(f"\n🏁 批次完成（{len(done)}/{total}）。批次清单已清空，本轮 ingest 结束。")
                INGEST_QUEUE_FILE.unlink()
                print("[STOP] 批次已全部完成。")
            else:
                next_file = pending[0]
                print(f"\n📊 批次进度：{len(done)}/{total} 完成，剩余 {remaining} 个文件：")
                for p in pending:
                    print(f"  - {p}")
                # [CONTINUE] 与 next_file 打印合并，确保在同一保护块内
                print(f"[CONTINUE] 处理下一文件：{next_file}")
        else:
            print("[STOP] 该文件不属于当前批次，批次不受影响。")

    except Exception:
        print("[STOP] 批次状态读取异常，请检查后手动决定下一步。")
...
def run_finalize(
    ...
    for d in ("sources", "entities", "concepts", "disambiguations", "syntheses"):
        ...
        if candidate.exists():
            ...
            break
    if subdir is None:
        print(f"⚠️ 未找到 Wiki/*/{slug}.md，请确认 Agent 已完成词条写入。")
        return 1

    raw_path = REPO_ROOT / raw_rel
    if not raw_path.exists():
        print(f"⚠️ 原始文件不存在: {raw_rel}")
        return 1
    ...
    if not standalone:
        try:
            file_hash_full = sha256_file(raw_path, truncate=None)
            path_hash = hashlib.sha256(str(raw_path).encode()).hexdigest()[:8]
            flag_file = REPO_ROOT / ".llm-wiki" / f".standalone-{path_hash}-{file_hash_full[:16]}.flag"
            if flag_file.exists():
                standalone = True
                flag_file.unlink()
                print("ℹ️ 检测到 standalone 标记文件，本次摄入不计入批次进度。")
        except Exception:
            pass

    # ── 确定是否需要写入日志/Git ──
    log_index = _load_log_index()
    already_ingested = (raw_rel in log_index and log_index[raw_rel] == file_hash)

    if already_ingested:
        print(f"ℹ️ {raw_rel} 已摄入且哈希未变，无新操作。")
    else:
        # Slug 冲突检测（仅对新摄入）
        if raw_rel not in log_index:
            conflict, existing = check_slug_conflict(slug, subdir, "ingest")
            existing_file = WIKI_DIR / subdir / f"{slug}.md"
            if conflict and not existing_file.exists():
                print(
                    f"[NEEDS_REVIEW] Slug 冲突：{subdir}/{slug}.md 已被其他来源占用（{existing}）。\n"
                    f"请指定新的 slug，或确认覆盖（若两篇文件属同一主题的不同版本）。"
                )
                return 2
        ...
        # 写入日志
        _write_log_entry(title, raw_rel, file_hash, slug)
        ...
        print(f"✅ 摄入完成：{title}（slug: {slug}，存入 Wiki/{subdir}/）")

    # ── 批次进度信号（所有路径均通过这里输出）──
    _update_batch_progress(raw_rel, standalone)
...
使用示例：
  python -m Tools.ingest --scan
  python -m Tools.ingest Raw/Sources/某论文.pdf
  python -m Tools.ingest Raw/Sources/某论文.pdf --force
  python -m Tools.ingest Raw/Sources/某论文.pdf --standalone
  python -m Tools.ingest --finalize 标题slug 显示标题 Raw/Sources/某论文.pdf --brief "一句话摘要"
  python -m Tools.ingest --finalize 标题slug 显示标题 Raw/Sources/某论文.pdf --brief "摘要" --standalone
"""
    )
    parser.add_argument("file",       nargs="?",      help="要摄入的文件路径（--scan 时忽略）")
    parser.add_argument("--scan",     action="store_true", help="扫描并列出所有待摄入的文件")
    parser.add_argument("--discuss",  action="store_true", help="启用单篇讨论模式")
    parser.add_argument("--force",    action="store_true", help="强制摄入超大文件（跳过大小检查）")
    parser.add_argument("--standalone", action="store_true",
                        help="本次摄入不参与批次进度，意图通过标记文件持久化")
    parser.add_argument("--finalize", nargs=3,
                        metavar=("SLUG", "TITLE", "RAW_REL"),
                        help="Agent 完成词条写入后回传 slug、title 和原始文件路径")
    parser.add_argument("--brief",    default="",
                        help="词条一句话摘要（≤30字），写入 index.md 条目（--finalize 时使用）")
    args = parser.parse_args()
    ...
    if not args.file:
        print("⚠️ 请指定要摄入的文件路径，或使用 --scan 查看待处理队列，例如：")
        print("  python -m Tools.ingest --scan")
        print("  python -m Tools.ingest Raw/Sources/某论文.pdf")
        return 1
...
def _get_last_ingest_date(raw_rel: str) -> str:
    ...
    log_text = read_file(LOG_FILE)
    pattern = re.compile(
        rf"^## \[(\d{{4}}-\d{{2}}-\d{{2}})\] ingest \| .+ \| \[\[{re.escape(raw_rel)}\]\]",
        re.MULTILINE
    )
    dates = [m.group(1) for m in pattern.finditer(log_text)]
...
def check_annotation_missing(pages_with_sources: list) -> Tuple[str, bool]:
    try:
        allowed = load_allowed_raw_dirs()
    except (FileNotFoundError, ValueError) as e:
        return f"加注缺失：无法加载目录配置（{e}），跳过检查。", False

    annotation_map = {d["name"]: d.get("annotation", "") for d in allowed}

    entries = []
    for page, sources, dir_name in pages_with_sources:
        expected_annotation = annotation_map.get(dir_name, "")
        if expected_annotation:
            content = read_file(page)
            entries.append({
                "page": page.relative_to(REPO_ROOT).as_posix(),
                "expected_annotation": f"（{expected_annotation}）",
                "sources": sources,
    ...
    for e in entries:
        ...
        context_lines.append(f"- 期望加注标记：{e['expected_annotation']}\n")
    ...
    if result == "[AGENT_PENDING]":
        return (
            ...
            f"请完成推理后重新运行 `python -m Tools.lint` 以完成审计。"
        ), True
...
def check_slug_conflicts() -> list:
    ...
    for slug, count in slug_counts.items():
        if count > 1:
            conflicts.append(f"slug '{slug}' 在 log.md 中出现 {count} 次，可能存在冲突")
    return conflicts


def check_blind_spots() -> Tuple[str, bool]:
    if not INDEX_FILE.exists():
    ...
    if result == "[AGENT_PENDING]":
        return (
            ...
            "结果将由 Agent 补充。请完成推理后重新运行 `python -m Tools.lint` 以完成审计。"
        ), True
    return f"知识盲区推荐：\n{result}", False
    ...
    if len(deg_values) >= 2:
        ...
        for n in active_nodes:
            if degrees.get(n["id"], 0) > threshold:
                issues.append(
                    f"上帝节点: {n['id']}（度数 {degrees[n['id']]}，μ+2σ={threshold:.1f}）"
                )
...
def run_lint(save: bool = False) -> int:
    today = date.today().isoformat()
    print("=" * 60)
    ...
    for page in all_wiki_pages():
        ...
        for rp in raw_paths:
            ...
            if len(parts) >= 2 and parts[1] in dir_names:
                pages_with_sources.append((page, raw_paths, parts[1]))
                break
    ...
    annotation_text, annotation_pending = check_annotation_missing(pages_with_sources)
    ...
    for category, issues in checks.items():
        status = "✅ 通过" if not issues else f"⚠️  {len(issues)} 项"
        print(f"\n[{category}] {status}")
    ...
    if pending_llm:
        print("\n" + "=" * 60)
        print("⚠️ 语义审计未完成：需要 Agent 处理 LLM 推理请求后重新运行 lint。")
        print("当前脚本不会记录最终日志，请完成推理后再次执行 `python -m Tools.lint`。")
        print("=" * 60)
        if save:
    ...
    if passed:
        ...
        print(f"⚠️  共发现 {severe} 项严重问题，建议处理后重新审计。")
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    sys.exit(run_lint(save=args.save))
```
...
运行方式：
  ...
  python -m Tools.query --apply --slug <slug> --answer "<答案文本>"
"""
...
def _save_synthesis(question: str, answer: str, slug: str) -> int:
    ...
    today = date.today().isoformat()
    escaped_title = question[:80].replace("\\", "\\\\").replace('"', '\\"')
    content = (
        f"---\n"
    ...
    print(f"✅ 已归档至 Wiki/syntheses/{slug}.md")
    return 0
...
def run_query(question: str, save: bool = False, slug: str = "",
    ...
    if open_questions:
        print(f"📌 已知开放问题（共 {len(open_questions)} 个）：")
    ...
    if wiki_hits:
        ...
        if raw_hits:
            print(f"\n⚠️ Raw 层命中 {len(raw_hits)} 个文件（知识盲区，Wiki 覆盖不足）：")
    ...
    if save:
        ...
        if answer == "[AGENT_PENDING]":
            ...
            print(
                "\n⚠️ 答案将由当前 Agent 生成。Agent 完成推理后，请执行：\n"
...
if __name__ == "__main__":
    ...
    if args.apply:
        if not args.answer or not args.slug:
            print("⚠️ --apply 需要同时提供 --slug 和 --answer")
            sys.exit(1)
        pending_data = {}
        if PENDING_QUERY.exists():
            try:
                ...
                pass
        question = args.question or pending_data.get("question", args.slug)
        sys.exit(run_query(question, save=True, slug=args.slug, apply_answer=args.answer))

    if not args.question:
        print("⚠️ 请提供查询问题，例如：python -m Tools.query '什么是纯粹理性批判'")
        sys.exit(1)
...
from Tools.common import (
    ...
    read_file, write_file, append_log,
    extract_wikilinks, all_wiki_pages, resolve_wikilink,
    agent_llm, git_add_commit, load_graph_json,
)
    ...
    edges_json = json.dumps([
        {
            "from": e["from"],
            "to":   e["to"],
            "color": {"color": "#ff4444" if e.get("relation") == "contradicts" else
                               "#aaaaaa" if e.get("type") == "INFERRED" else "#888888"},
...
def _generate_report(nodes: list, edges: list, inferred_edges: list) -> str:
    ...
    if god_nodes:
        for n in god_nodes:
            lines.append(f"- {n['id']}（度数 {n['degree']}，μ+2σ={threshold:.1f}）\n")
    else:
        lines.append("无上帝节点。\n")
...
def run_build_graph(
    ...
    if apply_inferred:
        if not INFER_RES.exists():
            print("⚠️ 未找到 Graph/.infer-results.json，无法应用推断边。")
            return 1
    ...
    if GRAPH_JSON.exists():
        ...
        degree_map = {}
        for e in all_edges:
            degree_map[e.get("from", "")] = degree_map.get(e.get("from", ""), 0) + 1
            degree_map[e.get("to", "")]   = degree_map.get(e.get("to", ""), 0) + 1
        for n in nodes:
            n["degree"] = degree_map.get(n["id"], 0)
        if apply_inferred:
            print(f"✅ 合并 {len(inferred_edges)} 条推断边。")
        else:
            print("ℹ️ 已加载现有 graph.json，如需要请使用 --apply-inferred 合并推断边。")
    else:
        if apply_inferred:
            print("⚠️ 未找到基础 Graph/graph.json，请先运行 build graph（不带 --apply-inferred）。")
            return 1
        ...
        print("✅ 基础 graph.json 已写入（Pass 1 结果，尚无推断边）。")
        ...
        if not no_infer:
            ...
            if result and result != "[AGENT_PENDING]":
                try:
                    ...
                    print(f"✅ Pass 2 完成，生成 {len(inferred_edges)} 条推断边。")
                except Exception as e:
                    print(f"⚠️ Pass 2 结果解析失败: {e}，跳过推断边（基础图谱已保存）。")
                    inferred_edges = []
    ...
    write_file(GRAPH_HTML, html)
    print(f"✅ graph.json 和 graph.html 已写入（{len(nodes)} 节点，{len(edges)} 边，{len(inferred_edges)} 推断边）。")

    if report:
        report_text = _generate_report(nodes, edges, inferred_edges)
        if save:
            write_file(GRAPH_REPORT, report_text)
            print("✅ 图谱健康报告已保存至 Graph/graph-report.md")
        else:
            print("\n" + report_text)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--apply-inferred", action="store_true", help="将 .infer-results.json 的推断边合并到 graph.json")
    args = parser.parse_args()
...
- 下一步操作提示：
  ...
  > 2. 在 AI 会话中说 `ingest` 触发摄入（Agent 先运行 `python -m Tools.ingest --scan` 获取队列，再逐文件调用 `python -m Tools.ingest <路径>`；临时处理的文件使用 `--standalone`；若标记文件创建失败需在 `--finalize` 中显式传递该标志；停止/继续信号看 `[STOP]`/`[CONTINUE]`）
  > 3. 说 `ingest --discuss` 切换为单篇讨论模式；完成词条写入后执行 `python -m Tools.ingest --finalize <slug> <title> <文件路径>`
  > 4. 说 `query: <问题>` 触发查询；归档时使用 `--save --slug <slug>`；若 Agent 推理待处理，使用 `--apply --slug <slug> --answer "<答案>"` 并建议补全问题文本
  > 5. 说 `health` 检查仓库健康状态（每次会话必跑）；若报告 qmd 集合未初始化，请运行 `qmd init --collection wiki Wiki/` 和 `qmd init --collection raw Raw/`
...
> **何时修订**：当某条规则反复产生不符合预期的结果，或某个工作流与实际使用习惯持续偏差时，与研究者讨论后修订。修订后更新上表（**必须填写"已知遗留问题"列**，若无则写"—"），并在 log.md 追加 `chore | AGENTS.md 修订 vX.X`。"已知遗留问题"列用于在修订记录层面预先承认本次修复是否干净，避免遗留冗余需等到下次评估才被发现。
...
| 指令 | 触发工作流 | 对应命令 | 说明 |
|------|-----------|---------|------|
| `ingest` | 摄入工作流（默认模式） | `python -m Tools.ingest --scan` → 逐文件 `python -m Tools.ingest <路径>` | 先运行 `--scan` 获取队列，再逐文件处理；严格遵守批次停止规则 |
| `ingest --standalone <file>` | 独立摄入 | `python -m Tools.ingest <file> --standalone` | 处理单个文件，完全不计入批次进度，不受队列影响 |
| `ingest --discuss` | 摄入工作流（单篇讨论模式） | `python -m Tools.ingest <路径> --discuss` | 每次处理 1 个文件，暂停讨论后通过 `--finalize` 恢复 |
| `ingest <file>` | 摄入特定文件（可能属于批次） | `python -m Tools.ingest <路径>` | 若文件在批次队列中，视为批次处理；否则临时处理，默认停止 |
| `ingest --finalize <slug> <title> <file> [--standalone]` | 摄入完成回传 | `python -m Tools.ingest --finalize <slug> <title> <路径> [--standalone]` | 词条写入完成后恢复摄入流程；脚本将通过标记文件自动识别 standalone 意图 |
| `query: <问题>` | 查询工作流 | `python -m Tools.query "<问题>"` | 先查 Wiki，盲区回退至 Raw，新洞见自动织网 |
...
| `lint` | 质量审计 | `python -m Tools.lint` | 在摄入批次完成后，若本批次摄入≥3 个文件即运行；否则可累积到下次较大批次或研究者显式要求时运行 |
| `build graph` | 图谱分析 | `python -m Tools.build_graph` | 可选扩展，建议知识库积累 50+ 词条后使用 |
...
- **Python 解释器**：优先使用 `python -m` 方式运行脚本，模块路径已通过包结构自动解析
...
Raw 层由研究者完全掌管，Agent 只读取，不做任何修改或移动。Raw 层子目录在 bootstrap 时由研究者确认，Agent 根据文件所在目录**自动确定加注类型**，无需猜测。研究者可自行在 Raw 子目录下创建更深层的归档目录，Agent 扫描时会递归发现，加注映射始终以 Raw 根下第一级子目录为准。

<raw-dir-table>

**【加注执行规则】**：

- `Sources/` 文件摄入时：他人文章中的推论、观点、概括 → 写入 Wiki 时强制加注 `（据文献）`
  - 示例：`某理论认为 X 是 Y 的充分条件（据文献）。`

- `Thoughts/` 文件摄入时：研究者自己的思考、判断、逻辑推演 → 写入 Wiki 时强制加注 `（个人思考）`
  - 示例：`休谟的同一性论证在数量与统一性边界处存在逻辑断层（个人思考）。`

- `Records/` 文件摄入时：所有主观内容、决策记录、经验总结 → 写入 Wiki 时强制加注 `（个人经验）`
...
3. 尝试读取相关图片；图片不可读时输出警告 `⚠️ 图片不可读: <路径>（原因：...）`，跳过该图片，不阻塞摄入流程
...
| 长文摘要（`sources/`） | 中文书名或文章标题.md（与原著标题保持一致） | `思考快与慢.md`、`纯粹理性批判.md` |
| 消歧词条（`disambiguations/`） | 中文词条名（流派）.md | `空（佛教）.md`、`空（道家）.md` |
| 综述页（`syntheses/`） | 中文语义短语.md（2–6字，概括查询主题） | `业力跨传统比较.md`、`动量策略失效条件.md` |

**【中文文件名强制规则】**：所有 Wiki 词条文件名必须使用中文（或保留原文的英文专有名词），严禁使用汉语拼音作为文件名。

**【slug 一致性原则】**：同一来源文件在不同会话中必须生成相同的 slug（词条中文文件名去后缀）。生成 slug 后立即记录至 log.md 的 `slug:` 字段。若检测到 slug 冲突（相同 slug 对应不同来源文件），输出警告，等待研究者手动确认重命名，不自动覆盖。
...
**`domain` 字段填写规范**：必须从 `.llm-wiki/domain-enum.json` 当前定义的合法枚举中选取（bootstrap 默认：<DOMAIN_ENUM>）。`health` 命令会自动校验该字段合法性。不确定时填写枚举中的最后一项（通常为 `其他`）。研究者可通过编辑 `.llm-wiki/domain-enum.json` 及 AGENTS.md 中的枚举列表来扩展领域。

> **注意**：`domain` 字段必须使用单行 YAML 标量格式（如 `domain: "哲学"`），请勿使用多行块标量写法（`>` 或 `|`），否则 health 检查会误报。
...
**触发**：`ingest` / `ingest --standalone <file>` / `ingest --discuss` / `ingest <file>`

默认摄入风格：**<DEFAULT_INGEST_MODE>**

> **注意**：无论默认模式是什么，另一种模式始终可通过显式参数临时切换。

**【两种摄入模式说明】**：

- **批量模式（默认）**：Agent 先运行 `python -m Tools.ingest --scan` 获取待摄入文件列表，然后依次对每个文件调用 `python -m Tools.ingest <路径>`，整批完成后汇报新建词条。

- **单篇讨论模式**：每次处理 1 个文件。Agent 调用 `python -m Tools.ingest <路径> --discuss`，脚本输出内容后暂停，等待研究者讨论；确认后 Agent 调用 `python -m Tools.ingest --finalize <slug> <title> <路径>` 完成后续步骤。
...
1. Agent 运行 `python -m Tools.ingest --scan` 获取待处理文件列表（生成批次清单 `.llm-wiki/ingest-queue.json`）
2. 对每个文件调用 `python -m Tools.ingest <路径>`
3. 脚本输出文件内容（前 3000 字符）及上下文信息
4. Agent 读取输出，写入/更新对应 Wiki 词条
5. 词条写入完成后，Agent 调用 `python -m Tools.ingest --finalize <slug> <title> <路径>`
6. 脚本自动完成 index.md 更新、日志追加、Git 提交、qmd 索引更新，并更新批次清单，输出停止或继续信号

**【`--standalone` 标志说明】**：当用户明确要求临时处理某个文件而不影响当前批次（或根本没有批次）时，Agent **必须**使用 `--standalone`。该标志会在 `ingest_file` 阶段创建一个持久化标记文件（`.llm-wiki/.standalone-*.flag`），确保即使 Agent 后续在 `--finalize` 中忘记显式传入该参数，`run_finalize` 也会通过检查标记文件自动识别并跳过批次更新。若标记文件因权限问题创建失败，脚本会输出警告，此时 Agent 需在 `--finalize` 中显式传递 `--standalone`；若忘记传递，该文件会被错误地当作批次文件处理，影响队列状态——发现此问题应检查 `.llm-wiki/ingest-queue.json` 的 `done` 列表是否有不该出现的项，手动编辑清除即可。

**【调用前检查】**：Agent 在调用 `python -m Tools.ingest <file>`（不带 `--standalone`）之前，若 `.llm-wiki/ingest-queue.json` 存在，应先 `read_file` 检查该文件路径是否在 `pending` 列表中。若在，且用户当前措辞是"看一下"、"先不正式处理"等探索性语言而非"处理"、"摄入"等确定性语言，Agent 应主动加上 `--standalone`，避免脚本将本次调用误判为批次推进的一部分。
...
- `[CONTINUE] 处理下一文件：<路径>` → 立即处理该路径，且**只能**处理该路径，不得自行从对话记忆中挑选其他文件
- `[STOP] <原因说明>` → 停止，不再继续处理任何文件

以脚本输出信号为准，无需额外判断。

> 📁 **提示**：Wiki 已更新并提交。若需将原始物料一并版本化：`git add Raw/ && git commit -m "raw: add source materials"`
...
**【冲突超时降级规则】**：若某条 `pending_review` 标记在 **连续 3 次健康检查**（即 3 次不同会话启动）中持续存在且研究者未主动裁决，Agent 可在不否定任何一方观点的情况下，将冲突双方均保留至词条正文，移除 `pending_review` 标记，并在 `log.md` 中追加 `## [日期] chore | auto-resolve: [[词条路径]]（超时降级，双方保留）`。此降级不会导致信息丢失，但可能稀释核心定义的明确性，研究者可随时手动裁决并回退。
...
3. **Agent 自行读取**命中文件内容，综合分析后给出答案
4. 盲区回退：Wiki 无命中时降级检索 Raw 集合
5. 生长机制：询问是否归档综述、是否摄入盲区文件、是否跨词条合成回哺

**归档注意事项**：若使用 `--save`，Agent 的 LLM 推理可能异步完成（见 `[AGENT_PENDING]` 提示）。此时脚本会保存上下文到 `.qmd-pending-query.json`，并提示使用 `--apply --slug <slug> --answer "<答案>"` 完成归档。**强烈建议在 `--apply` 命令中保留原始问题文本作为位置参数**，以防止 pending 文件意外丢失时综述标题退化。
...
检查项：空文件/存根页、索引同步、日志覆盖、Overview 占位、断链、Assets 断链、Raw 目录合规、哈希一致性（含 move/delete 追踪）、pending_review 扫描、Domain 合规（仅在校验 frontmatter 内）、qmd 集合初始化状态。

**关于"内容已修改"警告**：若 health 持续报告某个文件哈希与摄入记录不一致（常见于文件内容被手动编辑后），可通过**重新摄入该文件**（使用 `--standalone` 或将其放入批次）来更新哈希基准。
...
**频率**：每次摄入批次（`[STOP] 批次已全部完成`）后，若本批次摄入了 3 个以上文件，立即运行一次 lint；若本批次少于 3 个文件，可累积到下次较大批次或研究者显式要求时再运行。**不要求跨会话计数**——lint 是幂等的，频繁运行的唯一代价是 LLM 调用次数，宁可多跑。

检查项：孤儿页（豁免 sources/ 和 syntheses/）、过时综述、缺失实体页、稀疏页、Slug 冲突、图谱感知检查（可选）、加注缺失（LLM 辅助）、知识盲区推荐（LLM 辅助）。

**LLM 依赖项**：加注缺失和知识盲区检查需要 Agent LLM 推理。若推理未完成（脚本返回 exit code 2），`--save` 仍会写入一份**部分报告**，待推理完成重新运行后生成完整报告。
...
| standalone 标记文件 | `.llm-wiki/.standalone-*.flag` | `.gitignore` 屏蔽（临时意图传递） |
...
> 以下限制按**影响类型**分两组列出：第一组是设计上有意接受的简化，正常使用不会触发；第二组是工具执行失败时的应对路径。

### A. 设计简化（正常使用不需关注，极端写法才会遇到）

1. **`domain` 字段必须为单行 YAML 标量**：`health` 的 `check_domain_compliance` 用单行正则匹配 `domain:` 字段。若手动编辑 frontmatter 时使用多行块标量（`>` 或 `|`），会误报"domain 值非法"。请始终用单行写法 `domain: "哲学"`，已在 §3.1 中注明。

2. **lint 触发计数无跨会话持久化**：§八中"本批次摄入了 3 个以上文件才运行 lint"依赖 Agent 在单次会话内自行判断，不存在跨会话计数器。这是有意简化，避免引入额外状态文件。由于 lint 是幂等的，频繁运行无副作用，遇到不确定时直接运行即可。

### B. 工具执行失败的应对路径

3. **standalone 标记文件创建失败**：若 `ingest_file` 阶段因权限问题无法创建 `.llm-wiki/.standalone-*.flag`，脚本会打印警告。此时 Agent 需在后续 `--finalize` 中显式传递 `--standalone`。若忘记传递，该文件会被错误写入批次队列的 `done` 列表。排查方法：检查 `.llm-wiki/ingest-queue.json` 中的 `done` 列表是否有不该出现的项，手动编辑该 JSON 文件移除即可（注意 `.gitignore` 不入库，不影响 git 历史）。此问题与哈希一致性无关，health 报告的"哈希一致性"项不会提示该问题。
```

---

*本文件为修正版 bootstrap 提示词 v1.0。*
