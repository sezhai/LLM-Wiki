...
**当前版本**：v1.0  
**修订说明**：修正 bootstrap 阶段的多项边界问题与措辞歧义，提升健壮性。  
...
def _run_qmd(args: List[str], **kwargs):
    ...
    改进：当 PowerShell 降级路径也失败时，打印警告以便调试。"""
    if sys.platform == "win32":
        # Windows 下 qmd 可能是 .ps1 脚本，需通过 powershell 调用
        try:
...
def _strip_qmd_prefix(path: str) -> str:
    """将 qmd://wiki/entities/xxx.md → Wiki/entities/xxx.md。"""
...
def _search_qmd(question: str, collection: str = "wiki", n: int = 8) -> list:
    ...
    try:
        ...
        if result.returncode == 0:
            raw = result.stdout
            # Try UTF-8 first (no BOM), then UTF-16 (with BOM)
...
| Lint 回传缓存 | `Wiki/_annotation_check_result.md`、`Wiki/_blind_spot_result.md` | 保留供重跑使用 |
