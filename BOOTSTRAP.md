...
> 清单共 13 步（§1.0–§1.13），仅用于追踪状态，不重复步骤内容——每步的详细说明见对应小节。
...
# 以下均为可选项，默认委托当前 Agent 执行 LLM 推理，无需配置：
# LLM_BACKEND=/usr/local/bin/llm
...
def check_slug_conflict(
    ...
    """
    检查 slug 是否已存在于磁盘文件或 log.md 记录中（仅当对应不同 raw_rel 时视为冲突）。
    ...
    if LOG_FILE.exists():
        ...
        for _, raw_rel, _, rec_slug in parse_ingest_records(log_text):
            slug_to_raws.setdefault(rec_slug, set()).add(raw_rel)
        if slug in slug_to_raws and len(slug_to_raws[slug]) > 1:
...
def parse_log_slugs(log_text: str) -> Dict[str, str]:
    ...
    operation_type 为 'ingest' 或 'query-synthesis'，用于区分词条来源。
    同一 slug 多次出现时，以最后一条记录为准。
    """
    result: Dict[str, str] = {}
    # ingest 记录
...
def parse_moved_paths(log_text: str) -> Dict[str, str]:
    """
    解析 chore | move 记录，返回 {原始路径: 最新路径}，支持链式 move（A->B->C 映射为 A->C）。

    注意：本函数假设 log.md 中的 move 记录按时间顺序（从旧到新）排列。
    若记录乱序（如手动编辑 log），链式追踪可能不完整。
    """
    move_pattern = re.compile(
        r"^## \[\d{4}-\d{2}-\d{2}\] chore \| move: \[\[(.+?)\]\] -> \[\[(.+?)\]\]",
        re.MULTILINE
    )
    # origin_of[path] = 该路径的原始来源路径
    ...
    for m in move_pattern.finditer(log_text):
        ...
        # 更新映射：origin -> new_path
        moves[origin] = new_path
        # 记录 new_path 的原始来源（供后续链式 move 使用）
...
def agent_llm(prompt: str, output_var: str = "LLM_OUTPUT") -> str:
    ...
    - LLM_BACKEND_MODE=stdin（默认）：通过 stdin 传递 prompt
    - LLM_BACKEND_MODE=arg：通过位置参数传递 prompt
    ...
    if backend:
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                print(f"WARN: LLM_BACKEND call failed (exit {result.returncode}), falling back to Agent mode")
            else:
                print("WARN: LLM_BACKEND returned empty output, falling back to Agent mode")
        except Exception as e:
            print(f"WARN: LLM_BACKEND exception: {e}, falling back to Agent mode")
    ...
    print(f"Please complete the reasoning based on the prompt above and use the result as {output_var}.\n")
...
def git_add_commit(files: List[Path], message: str) -> None:
    ...
    try:
        ...
        print(f"WARN: Git operation failed: {e.stderr.decode() if e.stderr else str(e)}")
    except Exception as e:
        print(f"WARN: Git exception: {e}")
```
    ...
    if standalone:
        ...
        if queue_data is not None:
            ...
            if raw_rel in pending:
                ...
                _write_queue(queue_data)
                print(f"INFO: {raw_rel} 原属于当前批次，已标记为完成（standalone 模式下不计入 done，但从 pending 移除以避免队列卡死）。")
...
# 注意：标记文件仅基于文件路径哈希（不含内容哈希），因此即使 Raw 文件在
# ingest_file 和 --finalize 之间被修改，standalone 意图也不会丢失。
# ─────────────────────────────────────────────
def _standalone_flag_path(file_path: Path) -> Path:
    """
    基于文件的 REPO_ROOT 相对路径生成标记文件路径。
    仅使用路径字符串（不含内容哈希），确保文件修改后 standalone 意图不丢失。
    """
    try:
        rel = file_path.resolve().relative_to(REPO_ROOT).as_posix()
    except ValueError:
        rel = str(file_path)
    path_hash = hashlib.sha256(rel.encode()).hexdigest()[:24]
    flag_dir  = REPO_ROOT / ".llm-wiki"
    flag_dir.mkdir(parents=True, exist_ok=True)
    return flag_dir / f".standalone-{path_hash}.flag"


def _mark_standalone(file_path: Path) -> bool:
    """创建 standalone 标记文件。返回是否成功。"""
    try:
...
def run_finalize(
    ...
    if already_current:
        ...
        if normalized_rel not in log_index:
            conflict, existing_src = check_slug_conflict(slug, subdir, "ingest")
...
def check_annotation_missing(pages_with_sources: list) -> Tuple[str, bool]:
    ...
    修复：不再 break，而是为每个匹配来源目录分别追加检查条目。
    """
    try:
        allowed = load_allowed_raw_dirs()
    except (FileNotFoundError, ValueError) as e:
...
def run_lint(save: bool = False) -> int:
    ...
    # 收集所有来自需摄入目录的词条及其来源（不 break，全量收集）
    pages_with_sources = []
    for page in all_wiki_pages():
        content = read_file(page)
        fm = parse_frontmatter(content)
        ...
        if any(
            ...
            pages_with_sources.append((page, raw_paths, ""))  # 第三项保留兼容性，不再使用
    ...
    if args.apply:
        ...
        if PENDING_QUERY.exists():
            try:
                ...
                pass
        # 优先使用命令行提供的问题；若未提供则从 pending 文件读取；最后降级为 slug
...
**【`--standalone` 标志说明】**：当用户明确要求临时处理某个文件而不影响任何批次时，Agent **必须**使用 `--standalone`。该标志会在 `ingest_file` 阶段创建一个持久化标记文件（`.llm-wiki/.standalone-<路径哈希>.flag`）。标记文件**仅基于文件路径**生成，因此即使 Raw 文件在 `ingest_file` 和 `--finalize` 之间被修改，standalone 意图也不会丢失。
...
**检查项**：空文件/存根页、索引同步、日志覆盖（区分 ingest 和 query-synthesis 来源）、Overview 占位、断链、Assets 断链、Raw 目录合规、哈希一致性（含 move/delete 追踪）、pending_review 扫描、Domain 合规（仅在 frontmatter 内校验）、qmd 集合初始化状态、摄入批次残留。

关于"内容已修改"警告：若 health 持续报告某个文件哈希与摄入记录不一致，可通过重新摄入该文件来更新哈希基准。
...
**LLM 依赖项**：加注缺失和知识盲区检查需要 Agent LLM 推理。若推理未完成（脚本返回 exit code 2），`--save` 仍会写入一份部分报告，待推理完成重新运行后生成完整报告。
...
| standalone 标记文件 | `.llm-wiki/.standalone-<路径哈希>.flag` | .gitignore 屏蔽（临时意图传递，仅基于路径哈希） |
