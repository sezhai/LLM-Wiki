...
│   └── templates/         # 词条 frontmatter 模板（concept/entity/source_map/disambiguation/synthesis）
...
- 任何子目录下均不创建 README.md。`.llm-wiki/templates/` 目录在此步骤创建（模板文件在 §1.4 写入）

### 1.4 写入 AGENTS.md 及机器可读配置

> 写入 AGENTS.md 后，将本 bootstrap 文档 §2-A 中提供的五个代码块（去除外围 markdown 边界标记后）原样写入对应模板文件：`.llm-wiki/templates/concept.md`、`entity.md`、`source_map.md`、`disambiguation.md`、`synthesis.md`。AGENTS.md 正文已不再内联模板，改为指针引用。
...
AGENTS.md 内部不包含词条模板正文；模板已独立存放于 `.llm-wiki/templates/`（bootstrap 阶段从本文档 §2-A 提取）。
...
- Git 提交与推送：任何引发 Vault 文件变动的操作，在当前任务结束、收到 `[STOP]` 信号或回复研究者前，必须显式执行 `git add . && git commit -m "<操作类型>: <变更描述>" && git push` 完成本地与远程的同步闭环，严禁残留未提交的核心修改
...
新建词条前读取 `.llm-wiki/templates/<type>.md` 获取完整 frontmatter 模板（模板内容见 `.llm-wiki/templates/` 目录），不在此重复列出。模板类型对应关系：
| 词条类型 | 模板文件 |
|---------|---------|
| concepts/ | `concept.md` |
| entities/ | `entity.md` |
| sources/ | `source_map.md` |
| disambiguations/ | `disambiguation.md` |
...
**统一退出码**：`0`=通过，`1`=结构性问题需修复，`2`=挂起需 Agent 介入（lint: LLM 推理待回传；ingest: slug 冲突或 concept 覆盖缺失）。health/lint 共用此约定，ingest/query 仅复用部分值。非阻塞提示（日志覆盖、Overview 占位等）不改变退出码。
...
## 十一、临时文件处理规范
完整清单见 `.gitignore` 注释，Agent 在 `git add` 前无需额外清理。需关注的几个关键路径：
- 语义检查上下文/回传：`Wiki/_semantic_lint_context.md`、`Wiki/_annotation_check_result.md`、`Wiki/_blind_spot_result.md`
- 图谱推断中间文件：`Graph/.infer-context.json`、`Graph/.infer-results.json`、`Graph/.cache.json`
- 摄入队列：`.llm-wiki/ingest-queue.json`
- 运行报告：`Wiki/health-report.md`、`Wiki/lint-report.md`（用户确认后须手动 `git add -f` 写入）
...
### §2-A 模板文件（供 §1.4 bootstrap 阶段提取到 `.llm-wiki/templates/`）

以下五个代码块去掉外围 ```markdown 边界标记后，分别写入 `.llm-wiki/templates/concept.md`、`entity.md`、`source_map.md`、`disambiguation.md`、`synthesis.md`。

#### concept.md → `.llm-wiki/templates/concept.md`
...
era: ""
tags: []
sources: []  # 单行 JSON 数组，每项为 [[Raw/...]] 维基链接
related: []  # 可加 # supports / # contradicts 语义边注释
event_date:
last_updated: YYYY-MM-DD
...
#### entity.md → `.llm-wiki/templates/entity.md`
...
#### source_map.md → `.llm-wiki/templates/source_map.md`
...
#### disambiguation.md → `.llm-wiki/templates/disambiguation.md`
...
#### synthesis.md → `.llm-wiki/templates/synthesis.md`
