...
按顺序写入以下脚本文件，写入完成后逐一验证。**验证顺序**：

1. 先执行包导入冒烟测试：`python -c "import Tools"`，确保 `Tools/__init__.py` 存在且包结构正确。
2. 验证 `common.py`：`python -c "import Tools.common; print('common.py OK')"` 无报错即通过。
3. 逐一验证其他脚本：`python -m Tools.<script> --help`。
...
def scan_and_build_queue(candidates: List[str], mode: str = "normal") -> Optional[str]:
    ...
    若已有残留批次，直接恢复并忽略 mode 参数（保持原批次的 mode）。
    """
...
def _skip_and_advance(raw_rel: str, force: bool = False) -> None:
    queue_data = _read_queue()
    if queue_data is None:
        print("[STOP] 无批次清单。")
        return
...
def run_skip(raw_rel: str, force: bool = False) -> int:
    ...
    _skip_and_advance(normalized, force=force)
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LLM Wiki 摄入工具 v2.2",
...
""",
    ...
    parser.add_argument("targets", nargs="*", help="文件或目录路径（可混合；省略时扫描全库）")
...
def run_query(question: str, save: bool = False, slug: str = "",
    ...
    if qmd_hits_file:
        ...
        if hits_path.exists():
            try:
                raw_text = hits_path.read_text(encoding="utf-8")
            except UnicodeDecodeError:
                raw_text = hits_path.read_text(encoding="utf-16")
            try:
...
if __name__ == "__main__":
    ...
    if args.apply:
        ...
        # 若已存在且 update_flag 为 False，_save_synthesis 会输出清晰指引
...
| `ingest --discuss [文件或目录]` | 批量预读讨论模式 | `python -m Tools.ingest [路径...] --discuss` | 批量展示最多 5 个文件内容，统一讨论词条结构后逐篇 finalize，详见 §四 |
...
| `ingest [路径...] --discuss` | 与对应无 --discuss 版本相同 | 最多 5 个（一批） | 批量展示全部内容后统一讨论，讨论完成后逐篇 finalize |
