...
**当前版本**：v1.0（2026-06-19 综合修复：缓存/索引/去重/安全性/设计加固）
...
4. 若脚本输出 `[CONTINUE]`，Agent **必须立即无条件执行** `python -m Tools.ingest`（无参数），脚本自动从队列取下一个文件，输出其内容，回到步骤 2。**禁止**在收到 `[CONTINUE]` 后插入任何其他工具调用（如 health、lint、文件读取、思考评估等）——这些操作须在批次全部完成（`[STOP]`）之后进行。若输出 `[STOP]`，停止。
...
- `[CONTINUE] 立即执行：python -m Tools.ingest` → 执行纪律见上方"核心步骤"第 4 条。
