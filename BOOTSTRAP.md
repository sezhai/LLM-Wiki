...
def check_sparse_pages(link_graph: dict) -> list:
    issues = []
    excluded_stems = {"index", "log", "overview"}
    excluded_dirs = {"sources", "syntheses"}  # auto-generated pages, not expected to be densely linked
    for entry in link_graph.values():
        page = entry["page"]
        if page.stem in excluded_stems:
            continue
...
- 若 raw-mapping.json 中的 annotation 值被研究者修改，本规则自动适用新值，无需同步更新本文档
...
仅由 Raw/Sources/ 或其他 ingest=true 目录的文件触发。模板：
```markdown
---
title: ""
type: source_map
...
2. Agent 为该文件创建或更新对应的 Wiki 词条（source_map + 相关 concept/entity 等）。**每个新建词条（含 source_map）都需单独调用一次 `--finalize`**，不可仅 finalize concept/entity 而漏掉 source_map。
3. Agent 逐词条调用 `python -m Tools.ingest --finalize <slug> <title> <路径> [--brief "<摘要>"] [--no-concept ...]` 完成回传（source_map 应先于 concept/entity finalize）。脚本更新 index.md、日志、Git 提交、qmd 索引，并输出 `[CONTINUE]`（尚有文件）或 `[STOP]`（批次完成）。
