...
# 以下常量用于标识根目录下的特殊页面（仅前三项，health-report/lint-report 由 .gitignore 排除）
...
def _extract_raw_sources_from_frontmatter(fm: dict) -> Set[str]:
    """从 frontmatter 的 sources 字段（JSON 列表）中提取所有 Raw 路径。"""
    ...
    for item in sources_list:
        if isinstance(item, str):
            m = re.match(r"\[\[([^\]]+)\]\]", item)
            if m:
                raw_paths.add(normalize_path_str(m.group(1)))
            else:
                raw_paths.add(normalize_path_str(item))
    return raw_paths
...
def check_slug_conflict(
    ...
    for subdir in ("concepts", "entities", "sources", "syntheses", "disambiguations"):
        ...
        if target_path.exists():
            ...
            existing_raw = fm.get("raw_link", "").strip("[]") or None
    ...
    # ── 4. 日志中同一 slug 但不同 raw 来源的冲突 ──
    if log_text:
        # 使用 parse_moved_paths 将所有历史路径归一化为当前有效路径，避免 move 后误报
        moved = parse_moved_paths(log_text)
        seen_raws: Set[str] = set()
        for _, rec_raw, _, rec_slug in parse_ingest_records(log_text):
            if rec_slug == slug:
                # 若存在 move 记录，先映射到当前路径再比较
...
def parse_moved_paths(log_text: str) -> Dict[str, str]:
    """
    解析 move 记录，直接按日志出现的自然顺序处理。
    日志是追加写入的，因此自然顺序即为时间顺序，无需额外排序；
    同时避免了因同一天内多次移动而日期字符串无法区分先后的问题。
    
    重要：log.md 中 move 记录的行顺序决定了路径回溯的准确性，
    **严禁手动编辑 log.md 的 move 记录行**，否则可能导致 origin_of 链构建错误。
    """
...
def _validate_inferred_edges(inferred_edges: list, node_ids: set) -> list:
    """校验推断边的 from/to 是否真实存在于节点列表中，返回有效边列表。"""
    valid_edges = []
    ghost_count = 0
...
| `ingest --finalize <slug> <title> <file> [--force] [--no-concept ...]` | 摄入完成回传 | `python -m Tools.ingest --finalize <slug> <title> <路径> [--force] [--no-concept --no-concept-reason "<原因>"]` | 词条写入完成后推进批次；--force 跳过验证；--no-concept 豁免无可提取概念的文件 |
...
**discuss 模式流控说明**：当使用 `--discuss` 批量的预读并开始逐篇 finalize 时，观察脚本输出的信号（`[DISCUSS_CONTINUE]`/`[DISCUSS_READY]`/`[NEEDS_REVIEW]`/`[STOP]`）。若恢复了一个残留的普通批次，脚本会发出 WARN 但不改变队列模式，Agent 应先完成该普通批次再启动新的 discuss 批次，信号仍以队列当前模式为准。
...
检查项：孤儿页（豁免 sources/ 和 syntheses/）、过时综述、缺失实体页（Raw 源文件链接已被排除）、稀疏页（仅统计 Wiki 内部出站双链）、Slug 冲突（已使用 parse_moved_paths 归一化）、图谱感知检查（可选）、加注缺失（LLM 辅助，检查所有来源目录的加注合规性）、知识盲区推荐（LLM 辅助）。
...
**重要提醒**：若 Pass 2 挂起期间有新词条被摄入（通过 ingest），则旧有的推断边结果可能遗漏新词条与旧词条之间的关联。此时应重新运行完整的 `build_graph`（不加 `--apply-inferred`），以获取基于最新词条集合的推断边。
...
**禁止手动编辑 log.md 的 move 记录行**，以免破坏 parse_moved_paths 依赖的时序一致性。
...
- （可调参数） 摄入批次大小由 `Tools/ingest.py` 中的 `DEFAULT_BATCH_SIZE` 常量控制（默认 5），研究者可按需手动调整。
```
