...
│   └── tmp/               # 工具运行时临时文件（lint缓存、报告、查询中间产物等；gitignore 已屏蔽）
...
| **知识领域（domain）枚举** | 哲学、历史、宗教、文学、自然科学、技术、经济、其他 | 使用顿号分隔的列表，如需增删请说明。Agent 将据此生成 `.llm-wiki/domain-enum.json`，无需在 AGENTS.md 或模板文件中重复内嵌枚举内容。 |
...
- 任何子目录下均不创建 README.md。`.llm-wiki/tmp/` 目录在此步骤创建（由工具运行时自动使用）
...
type: concept
aliases: ["", "", ""]
domain: ""   # 须命中 .llm-wiki/domain-enum.json 枚举
subdomain: ""
era: ""
tags: []
sources: []  # 单行 JSON 数组，每项为 [[Raw/...]]
related: []  # 可加 # supports / # contradicts 语义边
event_date:
...
.obsidian/
workspace.json
# 工具运行时临时目录（lint 上下文/回传、health/lint 报告、查询中间产物）
.llm-wiki/tmp/
...
```
markitdown>=0.1.0,<0.4.0
pyyaml>=6.0
networkx>=3.0
```

markitdown 钉死 `>=0.1.0,<0.4.0`：该范围内 CLI 签名为 `markitdown <file>`（将输出写到 stdout）。0.4.0+ 可能改变 CLI 接口。若未来需升级，同步修改 ingest.py 中的调用方式。
...
TMP_DIR = REPO_ROOT / ".llm-wiki" / "tmp"
PENDING_QUERY = TMP_DIR / "qmd-pending-query.json"
...
def read_cached_llm_result(cache_path: Path, current_hash: str) -> Optional[str]:
    ...
    m = re.match(r"^# sha256:([a-f0-9]+)", content)
...
def write_cached_llm_result(cache_path: Path, result_hash: str, result_text: str) -> None:
    ...
    write_file(cache_path, f"# sha256:{result_hash}\n{result_text}")
...
def resolve_wikilink(name: str, title_alias_map: Optional[Dict[str, Path]] = None) -> Optional[Path]:
    ...
    clean = clean.split("#")[0]  # 先剥离锚点，否则 .md#Section 不以 .md 结尾导致残留
        ...
        for item in sources_val:
            if isinstance(item, str):
                ...
                if links:
                    ...
                    # 裸路径（无 [[...]] 包裹）时直接添加
                    cleaned = _clean_wikilink_str(item)
                    if cleaned:
                        raw_paths.add(normalize_path_str(cleaned))
...
def find_pages_by_raw_source(raw_rel: str) -> List[Path]:
    ...
    for page in all_wiki_pages():
        ...
        if not fm:
            continue
        raw_paths = _extract_raw_sources_from_frontmatter(fm)
...
def build_raw_source_index() -> Dict[str, List[Path]]:
    ...
    for page in all_wiki_pages():
        ...
        if page_str in cache_data and cache_data[page_str].get("mtime") == mtime:
            ...
            content = read_file(page)
            fm = parse_frontmatter(content)
            links = list(_extract_raw_sources_from_frontmatter(fm))
...
from Tools.common import (
    REPO_ROOT, WIKI_DIR, RAW_DIR, GRAPH_DIR, TOOLS_DIR, TMP_DIR,
    LOG_FILE, INDEX_FILE, OVERVIEW_FILE, RAW_MAPPING, DOMAIN_ENUM_FILE,
    INGEST_QUEUE_FILE,
    read_file, write_file, append_log,
    extract_wikilinks, all_wiki_pages, resolve_wikilink, build_title_alias_map,
    load_allowed_raw_dirs, load_domain_enum,
    sha256_file, list_pending_review, build_raw_source_index, find_pages_by_raw_source_indexed,
    git_add_commit, parse_log_slugs, parse_moved_paths,
    parse_ingest_records, parse_frontmatter, QMD_AVAILABLE, _run_qmd,
    _extract_raw_sources_from_frontmatter,
...
def check_log_coverage(pages: Dict[str, Path]) -> List[str]:
    ...
    for rel, p in pages.items():
        ...
        if stem not in slug_map:
            if subdir in ("concepts", "entities", "disambiguations"):
                content = read_file(p)
    ...
    for rel, p in pages.items():
        ...
        for link in links:
            if link.startswith("Raw/"):
                ...
                continue
            resolved = resolve_wikilink(link, title_alias_map=title_alias_map)
            if resolved is None:
    ...
    for raw_rel, recorded_hash in ingest_by_path.items():
        ...
        if not file_path.exists():
            ...
            if current_hash != recorded_hash:
                related = find_pages_by_raw_source_indexed(raw_rel, raw_source_index)
                if related and all(
...
def check_qmd_collections() -> List[str]:
    ...
    try:
        ...
        if "wiki" not in collections:
            issues.append("qmd wiki 集合未初始化，请运行: qmd collection add --name wiki Wiki/")
        if "raw" not in collections:
            issues.append("qmd raw 集合未初始化，请运行: qmd collection add --name raw Raw/")
    except Exception as e:
...
def run_health(save: bool = False, as_json: bool = False) -> int:
    ...
    if save:
        report_path = TMP_DIR / "health-report.md"
        ...
        print(f"\n报告已保存至 .llm-wiki/tmp/health-report.md")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save", action="store_true", help="将报告保存至 .llm-wiki/tmp/health-report.md")
...
def _advance_queue(raw_rel: str, subdir: str = "", slug: str = "",
    ...
    if no_concept and subdir == "sources" and slug and slug not in no_concept_slugs:
        ...
        # 批次内连续 3+ 文件使用 --no-concept 时发出警告，提示 Agent 重新评估
        # TODO: 当前仅为 print 提示，不阻断。若实际使用中发现 Agent 频繁无视此警告，
        # 可升级为要求调用方在 --finalize 时传入 --confirm-no-concept-streak 显式确认。
        if len(no_concept_slugs) >= 3:
            print(
                f"WARN: 本批次已有 {len(no_concept_slugs)} 个文件被豁免 concept/entity 检查。\n"
                f"  请逐文件复核是否确实都不含可提取概念，避免模式化套用 --no-concept。"
            )
    if raw_rel in done:
...
def _check_concept_coverage(done_rels: List[str], no_concept_slugs: List[str]) -> Tuple[bool, List[str], List[str]]:
    ...
    # no_concept_slugs 的写入（_advance_queue）保证在 _finalize_batch 调用
    # _check_concept_coverage 之前已完成。此处假设被豁免的 slug 均为 sources/ 类型，
    # 即 subdir == "sources" 的 --finalize --no-concept 调用写入的条目。
    # 若未来允许 # no_concept 标记非 sources 类型，需在此处同步放宽过滤条件。
...
def _check_batch_broken_links(queue_data: dict) -> List[Tuple[str, str]]:
    ...
    if not done:
        return []
    title_alias_map = build_title_alias_map()
    pages_to_scan: Set[Path] = set()
    for page in all_wiki_pages():
        page_str = page.as_posix()
        if "/sources/" not in page_str:
            continue
        content = read_file(page)
        fm = parse_frontmatter(content)
        if not fm:
        ...
        raw_link_val = fm.get("raw_link", "")
        if isinstance(raw_link_val, str):
            raw_links = extract_wikilinks(raw_link_val)
        elif isinstance(raw_link_val, list):
            raw_links = []
            for item in raw_link_val:
                if isinstance(item, str):
                    raw_links.extend(extract_wikilinks(item))
        else:
            raw_links = []
        if any(rl in done for rl in raw_links):
            pages_to_scan.add(page)
    for raw_rel in done:
        for page in find_pages_by_raw_source(raw_rel):
            rel = page.as_posix()
            if "/concepts/" in rel or "/entities/" in rel:
                pages_to_scan.add(page)
...
def _finalize_batch(queue_data: dict) -> None:
    ...
    # ── 批次末尾断链扫描 ──
    broken_links = _check_batch_broken_links(queue_data)
    if broken_links:
        target_counts: Dict[str, int] = {}
        for _, tgt in broken_links:
            target_counts[tgt] = target_counts.get(tgt, 0) + 1
        sorted_targets = sorted(target_counts.items(), key=lambda x: -x[1])
        print(f"\nWARN: 本批次相关词条中发现 {len(broken_links)} 条断链（可能因交叉引用指向了尚未创建的页面）：")
        for target, count in sorted_targets[:15]:
            print(f"  [{count}x] [[{target}]]")
        if len(sorted_targets) > 15:
            print(f"  ... 及其他 {len(sorted_targets) - 15} 个目标")
        print(f">>> 存在 {len(sorted_targets)} 个缺失目标页面。请考虑：① 运行 health 查看全貌；② 为被多次引用的概念创建存根页。")
    if remaining_candidates:
...
def _convert_to_markdown(file_path: Path) -> Tuple[bool, str]:
    ...
    if not MARKITDOWN_AVAILABLE:
        print(f"WARN: markitdown 未安装，无法转换 {file_path.name}。\n"
              f"请运行 pip install 'markitdown>=0.1.0,<0.4.0' 后重试。")
        return False, ""
    try:
...
def ingest_file(file_path: Path, discuss_mode: bool = False, force: bool = False) -> int:
    ...
    try:
        ...
        _skip_and_advance(rel, reason="文件读取失败")
        return 1
    ...
    if discuss_mode:
        ...
        return 0
    print(f"\n【Agent 操作步骤】\n"
          f"1. 视内容性质决定是否需要在 Wiki/sources/ 下创建摘要词条（source_map）：\n"
          f"   完整长文/原典/技术规范类文档建议创建；零散笔记、个人记录、对话片段等\n"
          f"   通常可跳过本步，直接进入步骤 2 处理 concept/entity\n"
...
def _write_ingest_log(title: str, raw_rel: str, file_hash: str, slug: str) -> None:
    if file_hash == "00000000000000000000000000000000":
        raise ValueError(
            f"拒绝写入零哈希日志条目: slug={slug}, raw_rel={raw_rel}。"
            f"请确保传入真实的 sha256 哈希值而非占位符。"
        )
    today = date.today().isoformat()
    entry = (
...
def _log_related_pages(raw_rel: str, file_hash: str) -> None:
    ...
    for page in related:
        ...
        if not (rel.startswith("Wiki/concepts/") or rel.startswith("Wiki/entities/") or rel.startswith("Wiki/sources/")):
            continue
        slug = page.stem
        ...
        append_log(entry)
        subdir = "concepts" if rel.startswith("Wiki/concepts/") else "entities" if rel.startswith("Wiki/entities/") else "sources"
...
def run_finalize(slug: str, title: str, raw_rel: str, brief: str = "",
    ...
    if not raw_rel or not raw_rel.strip():
        print("WARN: raw_rel is empty or whitespace, cannot finalize.")
        return 1
    ...
    if already_current:
        ...
        if slug not in existing_slugs:
            ...
            qmd_embed_wiki()
            print(f"OK: 摄入完成（新词条）：{title}（slug: {slug}，存入 Wiki/{subdir}/）")
        else:
            print(f"INFO: {raw_rel} 已摄入且哈希未变，slug 已有日志记录，跳过。")
...
from Tools.common import (
    REPO_ROOT, WIKI_DIR, RAW_DIR, LOG_FILE, INDEX_FILE, TMP_DIR,
    read_file, write_file, append_log,
    ...
    _content_hash, read_cached_llm_result, write_cached_llm_result,
    _extract_raw_sources_from_frontmatter,  # E: lint.py unify sources parsing 修复：导入常量
)
SEMANTIC_CONTEXT = TMP_DIR / "semantic-lint-context.md"
ANNOTATION_RESULT_FILE = TMP_DIR / "annotation-check-result.md"
BLIND_SPOT_RESULT_FILE = TMP_DIR / "blind-spot-result.md"
...
def check_orphan_pages(link_graph: dict) -> list:
    ...
    for entry in link_graph.values():
        ...
        if rel.startswith("syntheses/") or rel.startswith("sources/") or rel.startswith("disambiguations/"):
            continue
...
def check_sparse_pages(link_graph: dict) -> list:
    issues = []
    excluded_stems = {"index", "log", "overview"}
    excluded_dirs = {"sources", "syntheses", "disambiguations"}  # auto-generated pages and disambiguation hubs are not expected to be densely linked
    for entry in link_graph.values():
        page = entry["page"]
        if page.stem in excluded_stems:
            continue
        if page.parent.name in excluded_dirs:
            continue
    ...
    return issues
# _content_hash 已迁移至 common.py（与 read_cached_llm_result / write_cached_llm_result 配套使用）
def check_annotation_missing(pages_with_sources: list,
    ...
    entries = []
    for page, raw_paths in pages_with_sources:
        ...
        entries.append({
            ...
            "sources": raw_paths,
            "_body_preview": page_content[:500],  # 仅用于上下文展示，不参与 hash 计算
        })
    if not entries:
        return "加注缺失：无需检查（无来自需要加注目录的词条）。", False
    # 统一基于 entries 序列化计算哈希，保证缓存一致性
    # 计算哈希时排除 _body_preview，避免正文无关编辑导致缓存失效
    hash_entries = [{k: v for k, v in e.items() if not k.startswith('_')} for e in entries]
    entries_hash = _content_hash(json.dumps(hash_entries, sort_keys=True, ensure_ascii=False))
    result_text = ""
    ...
    cached = read_cached_llm_result(ANNOTATION_RESULT_FILE, entries_hash)
    ...
    lint_prompt = (
        f"请阅读 .llm-wiki/tmp/semantic-lint-context.md 中列出的 {len(entries)} 个词条，"
    ...
    if result == "[AGENT_PENDING]":
        return (
            f"加注缺失：已将 {len(entries)} 个词条写入 .llm-wiki/tmp/semantic-lint-context.md，"
...
def check_blind_spots(apply_answer_file: str = "", apply_answer_text: str = "") -> Tuple[str, bool]:
    ...
    if result_text:
        ...
        return f"知识盲区推荐：\n{result_text}", False
    cached = read_cached_llm_result(BLIND_SPOT_RESULT_FILE, current_hash)
    if cached is not None:
        return f"知识盲区推荐（缓存）：\n{cached}", False
    blind_spot_prompt = (
...
def run_lint(save: bool = False,
    ...
    if save:
        ...
        print(f"\n报告已保存至 .llm-wiki/tmp/lint-report.md")
    return EXIT_OK if passed else EXIT_BLOCKED  # Bug 3 修复：使用常量
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LLM Wiki 质量审计")
    parser.add_argument("--save", action="store_true", help="将报告保存至 .llm-wiki/tmp/lint-report.md")
...
from Tools.common import (
    ...
    read_file, write_file, append_log,
    extract_wikilinks, all_wiki_pages, resolve_wikilink,
    check_slug_conflict, git_add_commit, agent_llm, load_graph_json, _extract_raw_sources_from_frontmatter,
...
def _save_synthesis(question: str, answer: str, slug: str,
    ...
    if is_existing_synthesis and update:
        ...
        # 复用 common.py 的扁平化解析，正确处理历史脏数据（嵌套列表等）
        old_sources = list(_extract_raw_sources_from_frontmatter(old_fm))
        # 合并去重（统一使用 wikilink 格式）
...
def run_query(question: str, save: bool = False, slug: str = "",
    ...
    # CJK bigram fallback: Chinese text lacks word delimiters, so long tokens
    # (e.g. "康德的纯粹理性批判讲了什么") need sub-tokenization for recall
    cjk_char = re.compile(r"[\u4e00-\u9fff\u3400-\u4dbf]")
    extra = []
    for kw in keywords:
        chars = cjk_char.findall(kw)
        if len(chars) >= 4:
            extra += ["".join(chars[i:i+2]) for i in range(len(chars) - 1)]
    keywords += extra
    synth_dir = WIKI_DIR / "syntheses"
    open_questions = []
    if synth_dir.exists():
...
if __name__ == "__main__":
    ...
    if args.apply:
        ...
        if not question:
            print(f"WARN: 未能获取原始问题文本，综述标题将降级为 slug: {args.slug}。")
            question = args.slug
        ...
        result = run_query(question, save=True, slug=args.slug,
                           apply_answer=answer_text, source_hits=source_hits,
                           update=update_flag)
        if result == 0 and PENDING_QUERY.exists():
            try:
                PENDING_QUERY.unlink()
            except OSError:
                pass
        sys.exit(result)
...
"""
import re, json, argparse, statistics, hashlib, webbrowser, sys
from pathlib import Path
from datetime import date
...
try:
    ...
    HAS_NETWORKX = False
    print("Warning: networkx not installed. Community detection disabled.", file=sys.stderr)

from Tools.common import (
    REPO_ROOT, WIKI_DIR, GRAPH_DIR, TMP_DIR,
    read_file, append_log, init_env, ensure_utf8,
    extract_wikilinks, all_wiki_pages, agent_llm,
)

GRAPH_JSON = GRAPH_DIR / "graph.json"
GRAPH_HTML = GRAPH_DIR / "graph.html"
CACHE_FILE = TMP_DIR / "graph-cache.json"
INFERRED_EDGES_FILE = TMP_DIR / "graph-inferred-edges.jsonl"
CONTEXT_FILE = TMP_DIR / "graph-infer-context.json"
RESULTS_FILE = TMP_DIR / "graph-infer-results.json"

TYPE_COLORS = {"source": "#4CAF50", "entity": "#2196F3", "concept": "#FF9800", "synthesis": "#9C27B0", "unknown": "#9E9E9E"}
...
def build_extracted_edges(pages: list[Path]) -> list[dict]:
    ...
    edges, seen = [], set()
    for p in pages:
        src, content = page_id(p), read_file(p)
        rels, in_related = {}, False
        for line in content.splitlines():
            stripped = line.strip()
            if re.match(r'^related\s*:', stripped): in_related = True; continue
            if in_related:
                if stripped and not stripped.startswith('-') and ':' in stripped and not stripped.startswith('#'): break
                if m := re.search(r'\[\[([^\]]+)\]\].*?#\s*(supports|contradicts|extends|depends_on)', stripped, re.I):
                    rels[m.group(1).strip().removeprefix('Wiki/').removesuffix('.md')] = m.group(2).lower()

        for link in extract_wikilinks(content):
            norm = link.lower().replace('\\', '/').removeprefix('wiki/')
            if norm.startswith('raw/'): continue
            norm_no_ext = norm.removesuffix('.md')
            target = stem_map.get(norm_no_ext) or path_map.get(norm_no_ext)
            if target and target != src and (src, target) not in seen:
                seen.add((src, target))
                edges.append({
                    "id": edge_id(src, target, "EXTRACTED"), "from": src, "to": target, 
                    "type": "EXTRACTED", "relation": rels.get(norm_no_ext, "link"),
...
def build_inferred_edges(pages: list[Path], cache: dict, resume: bool = True) -> tuple[list[dict], bool]:
    edges, completed = [], set()
    if resume and INFERRED_EDGES_FILE.exists():
        for line in INFERRED_EDGES_FILE.read_text(encoding="utf-8").splitlines():
            if not line.strip(): continue
            try:
                rec = json.loads(line)
                completed.add(rec["page_id"])
                for e in rec.get("edges", []):
                    rel_type = e.get("type", "INFERRED")
                    edges.append({
                        "id": e.get("id", edge_id(e["from"], e["to"], rel_type)),
                        "from": e["from"], "to": e["to"], "type": rel_type,
                        "relationship": e.get("relationship", e.get("title", "")),
                        "color": e.get("color", EDGE_COLORS.get(rel_type, EDGE_COLORS["INFERRED"])),
    ...
    changed, ctx_nodes = [], []
    for p in pages:
        pid, content = page_id(p), read_file(p)
        h = sha256(content)
        ...
        if isinstance(entry, dict) and entry.get("hash") == h:
            for rel in entry.get("edges", []):
                ...
                edges.append({
                    "id": edge_id(pid, rel["to"], rel_type), "from": pid, "to": rel["to"], 
                    "type": rel_type, "relationship": rel.get("relationship", rel.get("title", "")),
            ...
            changed.append(p)
            body = re.sub(r"^---\n.*?\n---\n?", "", content, flags=re.DOTALL)
            ctx_nodes.append({"id": pid, "type": extract_frontmatter_type(content), "preview": body[:500].strip()})

    if not changed: return edges, True

    TMP_DIR.mkdir(parents=True, exist_ok=True)
    CONTEXT_FILE.write_text(json.dumps({"nodes": ctx_nodes}, indent=2), encoding="utf-8")
    
    prompt = f"Analyze these wiki pages and infer implicit relationships (max 3 per page):\n" + \
             "\n".join(f"- {n['id']} ({n['type']})" for n in ctx_nodes) + \
             "\nReturn JSON with 'edges' array: [{from, to, relationship, confidence, type}]. Output strictly valid JSON."
    
    raw = agent_llm(prompt, output_var="INFERRED_EDGES")
    if raw == "[AGENT_PENDING]":
        print(f"\nPass 2 suspended: context written to {CONTEXT_FILE}. Complete inference and run --apply-inferred.")
        return edges, False
        
    try:
        new_edges = json.loads(re.sub(r"^```(?:json)?\s*|\s*```$", "", raw)).get("edges", [])
    except Exception as e:
        print(f"WARN: Failed to parse inference: {e}")
        return edges, False

    node_ids, from_edges = {page_id(p) for p in pages}, {}
    for e in new_edges:
        if e.get("from") in node_ids and e.get("to") in node_ids:
            conf = float(e.get("confidence", 0.7))
            rel_type = e.get("type", "INFERRED" if conf >= 0.7 else "AMBIGUOUS")
            valid = {
                "id": edge_id(e["from"], e["to"], rel_type), "from": e["from"], "to": e["to"],
                "type": rel_type, "relationship": e.get("relationship", ""),
                "color": EDGE_COLORS.get(rel_type, EDGE_COLORS["INFERRED"]), "confidence": conf,
            }
            edges.append(valid)
            from_edges.setdefault(valid["from"], []).append(valid)

    for pid, elist in from_edges.items():
        page_path = next((p for p in pages if page_id(p) == pid), None)
        if page_path:
            cache[str(page_path)] = {
                "hash": sha256(read_file(page_path)),
                "edges": [{"to": x["to"], "relationship": x["relationship"], "confidence": x["confidence"], "type": x["type"]} for x in elist]
            }
            with open(INFERRED_EDGES_FILE, "a", encoding="utf-8") as f:
                f.write(json.dumps({"page_id": pid, "edges": elist, "ts": date.today().isoformat()}) + "\n")
...
def generate_report(nodes: list[dict], edges: list[dict], communities: dict[str, int], pages: list[Path]) -> str:
    ...
    degs = dict(G.degree())
    orphans = [n for n, d in degs.items() if d == 0]
    mean_deg, std_deg = statistics.mean(degs.values()) if degs else 0, statistics.stdev(degs.values()) if len(degs) > 1 else 0
    gods = [n for n, d in degs.items() if d > mean_deg + 2 * std_deg]
    
    lines = [
        f"# Graph Insights ({date.today().isoformat()})",
        f"- **Nodes:** {len(nodes)}, **Edges:** {len(edges)} (Density: {nx.density(G):.4f})",
        f"- **Communities:** {len(set(communities.values())) if communities else 0}",
        f"- **Orphans ({len(orphans)}):** {', '.join(orphans[:10])}{'...' if len(orphans)>10 else ''}",
        f"- **God Nodes ({len(gods)}):** {', '.join(gods[:10])}{'...' if len(gods)>10 else ''}"
    ]

    if pages:
        existing = {p.stem.lower() for p in pages} | {page_id(p).lower() for p in pages}
        refs = {}
        for p in pages:
            for link in extract_wikilinks(read_file(p)):
                norm = link.lower().replace('\\', '/').removeprefix('wiki/').removesuffix('.md')
                if not norm.startswith('raw/') and norm not in existing:
                    refs.setdefault(link, set()).add(page_id(p))
        phantoms = sorted([(k, len(v)) for k, v in refs.items() if len(v) >= 2], key=lambda x: -x[1])
        if phantoms:
...
def build_graph(infer: bool = True, open_browser: bool = False, clean: bool = False,
    ...
    if not pages: return print("Wiki is empty.")
        
    GRAPH_DIR.mkdir(parents=True, exist_ok=True)
    if clean and INFERRED_EDGES_FILE.exists(): INFERRED_EDGES_FILE.unlink()
        
    cache = load_cache()
    nodes, edges = build_nodes(pages), build_extracted_edges(pages)
    inference_complete, inferred = True, []
    
    if apply_inferred:
        if RESULTS_FILE.exists():
            try:
                data, node_ids = json.loads(RESULTS_FILE.read_text(encoding="utf-8")), {n["id"] for n in nodes}
                for e in data.get("inferred_edges", []):
                    if e.get("from") in node_ids and e.get("to") in node_ids:
                        rel_type = e.get("type", "INFERRED")
                        inferred.append({
                            "id": edge_id(e["from"], e["to"], rel_type), "from": e["from"], "to": e["to"],
                            "type": rel_type, "confidence": float(e.get("confidence", 0.7)),
                            "relationship": e.get("relationship", ""), "color": EDGE_COLORS.get(rel_type, EDGE_COLORS["INFERRED"])
                        })
            except Exception as e: print(f"WARN: apply inferred failed: {e}"); inference_complete = False
        else: print("WARN: --apply-inferred used but results file not found."); inference_complete = False
    elif infer:
        inferred, inference_complete = build_inferred_edges(pages, cache, resume=not clean)
        save_cache(cache)

    edges = deduplicate_edges(edges + inferred)
    communities, degree_map = {}, {}
    ...
    if HAS_NETWORKX:
        ...
        G.add_edges_from((e["from"], e["to"]) for e in edges)
        try:
            for i, comm in enumerate(nx_community.louvain_communities(G, seed=42)):
                for node in comm: communities[node] = i
    ...
    for node in nodes:
        ...
        if node["group"] >= 0: node["color"] = COMMUNITY_COLORS[node["group"] % len(COMMUNITY_COLORS)]
        node["value"] = degree_map.get(node["id"], 0) + 1

    GRAPH_JSON.write_text(json.dumps({
        "nodes": nodes, "edges": [e for e in edges if e.get("type") == "EXTRACTED"],
        "inferred_edges": [e for e in edges if e.get("type") in ("INFERRED", "AMBIGUOUS")],
        "built": date.today().isoformat(), "inference_complete": inference_complete,
    }, indent=2, ensure_ascii=False), encoding="utf-8")
    
    # 替换 </script> 防止节点内容破坏 HTML
    nodes_json = json.dumps(nodes, ensure_ascii=False).replace("</script>", "<\\/script>")
    edges_json = json.dumps(edges, ensure_ascii=False).replace("</script>", "<\\/script>")

    html = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>LLM Wiki Graph</title>
<script src="https://unpkg.com/vis-network/standalone/umd/vis-network.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script>
<style>
  body {{ margin: 0; display: flex; background: #1a1a2e; color: #eee; font-family: sans-serif; height: 100vh; overflow: hidden; }}
  #graph {{ flex: 1; height: 100%; }}
  #drawer {{ width: clamp(300px, 33vw, 600px); background: #151525; border-left: 1px solid #333; padding: 20px; overflow-y: auto; display: none; box-sizing: border-box; box-shadow: -5px 0 15px rgba(0,0,0,0.5); }}
  #drawer.open {{ display: block; }}
  .close-btn {{ cursor: pointer; float: right; font-size: 24px; color: #888; line-height: 1; }}
  .close-btn:hover {{ color: #fff; }}
  h2 {{ margin-top: 0; font-size: 22px; }}
  pre {{ background: rgba(255,255,255,0.05); padding: 12px; border-radius: 6px; overflow-x: auto; font-size: 13px; }}
  code {{ font-family: monospace; background: rgba(255,255,255,0.1); padding: 2px 4px; border-radius: 4px; }}
  pre code {{ background: transparent; padding: 0; }}
  a, .wikilink {{ color: #86c8ff; font-weight: bold; text-decoration: none; }}
</style>
</head><body>
<div id="graph"></div>
<div id="drawer">
  ...
  <h2 id="d-title"></h2>
  <div style="color: #999; font-size: 13px; margin-bottom: 15px;" id="d-meta"></div>
  <div id="d-content" style="line-height: 1.6; font-size: 14px; color: #ccc;"></div>
</div>
<script>
const nodes = new vis.DataSet({nodes_json});
const edges = new vis.DataSet({edges_json});
const network = new vis.Network(document.getElementById("graph"), {{nodes, edges}}, {{
  nodes: {{ shape: "dot", font: {{ color: "#ddd" }}, scaling: {{ min: 10, max: 30 }} }},
  edges: {{ width: 1, arrows: "to", color: {{ inherit: false }}, hoverWidth: 2 }},
...
network.on("click", params => {{
  if (params.nodes.length > 0) {{
    ...
    document.getElementById("d-meta").textContent = node.type + " | " + node.path;
    let md = (node.markdown || "").replace(/^---\\n[\\s\\S]*?\\n---\\n?/, "");
    md = md.replace(/\\[\\[([^\\]]+)\\]\\]/g, '<span class="wikilink">[[$1]]</span>');
    document.getElementById("d-content").innerHTML = marked.parse(md);
...
function closeDrawer() {{ document.getElementById("drawer").classList.remove("open"); }}
    ...
    append_log(f"## [{date.today().isoformat()}] graph | 重建 ({len(nodes)} 节点, {len(edges)} 边)")
    
    if report and HAS_NETWORKX:
        report_text = generate_report(nodes, edges, communities, pages)
        print("\n" + report_text)
        if save: (TMP_DIR / "graph-report.md").write_text(report_text, encoding="utf-8")
            
    if open_browser: webbrowser.open(str(GRAPH_HTML.resolve()))
    if not inference_complete: print("\n[NEEDS_REVIEW] Inference incomplete. Run with --apply-inferred later.")

if __name__ == "__main__":
    init_env()
    ensure_utf8()
    parser = argparse.ArgumentParser(description="Build LLM Wiki knowledge graph")
    parser.add_argument("--no-infer", action="store_true", help="Skip semantic inference")
    parser.add_argument("--open", action="store_true", help="Open graph.html in browser")
    parser.add_argument("--clean", action="store_true", help="Force full re-inference")
    parser.add_argument("--report", action="store_true", help="Generate health report")
    parser.add_argument("--save", action="store_true", help="Save report to tmp/graph-report.md")
    parser.add_argument("--apply-inferred", action="store_true", help="Merge inferred edges")
    args = parser.parse_args()
    build_graph(not args.no_infer, args.open, args.clean, args.report, args.save, args.apply_inferred)
```
...
AGENTS.md 内部包含完整词条模板（§3.1–§3.5），Agent 创建词条时直接参考即可，无需读取外部模板文件。
...
qmd 集合提示：若 health 报告中出现 "qmd 集合未初始化" 警告，请在开始前提醒研究者运行对应的 qmd collection add 命令。
## 快速触发指令
| 指令 | 触发工作流 | 对应命令 | 说明 |
|------|-----------|---------|------|
...
| `query: <问题>` | 查询工作流（两步） | Step 1：`qmd query "<问题>" --collection wiki --format json -n 8 > .llm-wiki/tmp/qmd-hits.json`；Step 2：`python -m Tools.query "<问题>" --qmd-hits .llm-wiki/tmp/qmd-hits.json` | qmd 不可用或失败时直接执行 Step 2（脚本自动降级）。保存结果追加 `--save --slug <slug>`；Agent 推理完成后归档使用 `--apply --slug <slug> --answer "<答案>"`（强烈建议带上原始问题文本）。 |
...
- 强制 Git 提交与推送：任何引发 Vault 文件变动的操作，在当前任务结束、收到 `[STOP]` 信号或回复研究者前，必须显式执行 `git add . && git commit -m "<操作类型>: <变更描述>" && git push` 完成本地与远程的同步闭环，严禁残留未提交的核心修改
...
【加注执行规则】（具体 annotation 文字以 raw-mapping.json 的 annotation 字段值为唯一权威来源）：
- annotation 非空的目录：摄入时所有他人观点/推论/主观内容写入 Wiki 时强制加注，格式为"（<annotation字段值>）"
- annotation 为 null 的目录（如 Assets）：不独立摄入，不加注
- raw-mapping.json 修改后规则自动适用新值，无需同步更新本文档
【Assets/附件定位与图片读取规则】：annotation 为 null 的目录（默认 Assets/，具体见 `.llm-wiki/raw-mapping.json`）不独立摄入。当摄入含图片引用的 Markdown 文件时，Agent 在该次摄入流程内读取被引用的图片作为补充上下文：
1. 先完整读取文本内容，按正常流程生成词条草稿
2. 识别文中 `![[...]]` 或 `![](Raw/<asset_dir>/...)` 引用的图片路径（`<asset_dir>` 为 annotation=null 的目录名）
3. 尝试读取相关图片；图片不可读时输出警告 `WARN: 图片不可读: <路径>（原因：...）`，跳过该图片，不阻塞摄入流程
4. 将可读图片信息补充进对应词条的相关章节，标注 `[[图示补充]]`
【四条红灯禁令】：
- 禁止主观裁判：不得独立判断哪种研究观点或诠释"更正确"
- 禁止自我引用：严禁将 Agent 自己写就的总结性言论作为推导新事实的"客观证据"
- 禁止推断型事实填补：来源信息缺失时不得推断补全，缺失即空白或标注 `[[来源缺失，待补充]]`
- 查询合成加注：跨词条合成产生的新推论必须加注 `（据查询合成，未经来源验证）`
## 三、词条元数据与排版规范
...
domain 字段填写规范：必须从 `.llm-wiki/domain-enum.json` 当前定义的合法枚举中选取。health 命令会自动校验该字段合法性。不确定时填写枚举中的最后一项（通常为"其他"）。研究者可通过编辑 `.llm-wiki/domain-enum.json` 来扩展领域。
...
是否需要为某文件创建 source_map，由内容性质而非所在目录决定：完整长文/原典/技术规范等结构化长文档建议创建；零散笔记、个人记录、对话片段等通常无需 source_map，可直接转入 concept/entity 词条。适用时使用以下模板：
```markdown
---
title: ""
type: source_map
...
## 核心抽取概念
（仅允许 concepts/ 链接；写入前自问：目标在 concepts/ 下？是可泛化的抽象？去掉会丢语义？三者皆"是"才保留）
...
- 文件名必须中文（保留英文专有名词除外），严禁拼音
- 全局 slug 唯一，建表前调用 check_slug_conflict
- Concept/Entity 判断："这概念在其他场合还适用吗"→是→concept；只能问"这件事发生过吗"→entity
- domain 字段须命中 `.llm-wiki/domain-enum.json` 枚举
- 多切片著作：每片独立 slug（`书名_切片标记`），总纲页 raw_link 留空，不占用任何切片 slug
- `## 核心抽取概念` 节严禁出现 entities/sources/disambiguations 链接，仅限 concepts/
...
# 正确：数组直接传递（注意：若未匹配到文件，$files 为空数组，直接调用会进入"恢复已有队列"模式而非报错，建议先检查）
# if ($files.Count -eq 0) { Write-Warning "未匹配到文件，请检查关键字" }
...
2. Agent 为该文件创建或更新对应的 Wiki 词条（source_map + 相关 concept/entity 等，全部应在调用 --finalize 前完成文件写入，并确保 concept/entity 的 frontmatter sources 字段已引用该 raw_rel）。**仅需对该 raw_rel 显式调用一次 --finalize**——通常即 source_map 的 slug。该次调用成功后，脚本会自动扫描所有 frontmatter sources 字段已引用该 raw_rel、但尚未写入日志的 concept/entity/disambiguation 页面，将其一并补记入 log.md 与 index.md，**无需**为这些关联词条再单独调用 --finalize。
   - 例外：若本文件本身不产出 source_map（例如该词条就是一个独立的 concept/entity，没有对应的长文摘要页），直接对该 concept/entity 调用一次 --finalize 即可推进队列。
3. Agent 调用一次 python -m Tools.ingest --finalize <slug> <title> <路径> [--brief "<摘要>"] [--no-concept ...] 完成回传（slug 通常为 source_map 的 slug；若属于上一步的例外情形，则为该唯一 concept/entity 的 slug）。脚本更新 index.md、日志、Git 提交、qmd 索引，自动补记关联词条，并输出 [CONTINUE]（尚有文件）或 [STOP]（批次完成）。批次队列以 raw_rel 为单位推进；对同一 raw_rel 重复调用 --finalize 不会报错，但已记录的 slug 会被直接跳过。
4. **【信号纪律】** 每次 `--finalize`/`--skip` 后脚本输出以下之一：
   - `[CONTINUE]` → 立即无条件执行 `python -m Tools.ingest`（无参数），**不得**在中途插入 health/lint/读文件等任何其他调用。状态机完整性依赖此纪律。
   - `[STOP] <原因>` → 停止，即便全库仍有待摄入文件也不主动继续，除非用户再次触发。
...
**【--no-concept 硬约束】** — 使用前必须三问皆否，否则禁止豁免：

1. 此文件是否存在可提取的抽象概念/方法论/术语体系？
2. 标题或标签是否暗示某种思想体系？
3. 是否指向一个具有唯一历史身份的实体（人物、著作、机构、事件）？
（entity 词条已存在 ≠ 概念豁免理由，二者互不相关，须独立判断）

**逐文件评估**：禁止模式化套用，遇到"摘录"类连续文件尤须警惕——这是曾出过事故的模式，永久禁止。

**时机说明**：是否需要 --no-concept 是基于该文件内容本身的判断，与该文件是否为本批次"最后一个" raw_rel 无关——Tools/ingest.py 中 --no-concept 的登记逻辑在每次 --finalize 调用时立即生效，不依赖队列位置。Agent 无需、也无法在 finalize 某文件时准确预判自己是否处于批次末位，判断结论应在该文件 finalize 时即给出并传入参数。批次末尾验证只是汇总检查彼时 done 列表的整体覆盖情况；若因当时未传入 --no-concept 而被 [NEEDS_REVIEW] 打回，只需在重新执行该文件的 --finalize 时补上 --no-concept --no-concept-reason "<原因>" 即可，不会损坏已写入的词条或日志。
...
- 消歧导航页及其子专页若在单次摄入中一并创建：(1) --finalize 的 slug 参数只能登记其中一个，其余文件会触发 health 日志覆盖缺失提示，这是 NON_BLOCKING/非阻塞项，不影响 health 退出码；(2) 消歧导航页及其子专页本身在创建初期通常还没有其他词条的入站双链，子专页之间互相引用也往往不足 2 条出站双链——lint 的孤儿页、稀疏页检查已对 disambiguations/ 目录整体豁免（与 sources/、syntheses/ 同等处理），不会因此产生提示或影响 lint 退出码。
## 六、查询工作流（Query Workflow）
触发：`query: <问题>` → 按两步执行（qmd 可用时先执行 Step 1，否则直接执行 Step 2）
执行步骤：
1. **【强制】qmd 语义检索**：
   ```bash
   qmd query "<问题>" --collection wiki --format json -n 8 > .llm-wiki/tmp/qmd-hits.json
   ```
   降级规则（按优先级，任一条件满足即跳过本步直接执行 Step 2）：
   - qmd 未安装 → 跳过，注明"qmd 未安装"
   - qmd 命令报错（集合未初始化、权限不足、任何非零退出码）→ 跳过，注明具体错误；不得因 Step 1 报错停止整个查询流程
   - `.llm-wiki/tmp/qmd-hits.json` 写入成功但内容为空数组 `[]` → 正常传入 Step 2，脚本自动降级关键词匹配
2. 执行查询脚本：
   ```bash
   python -m Tools.query "<问题>" --qmd-hits .llm-wiki/tmp/qmd-hits.json
   ```
...
【qmd 临时输出文件】：`.llm-wiki/tmp/qmd-hits.json` 为单次查询的中间产物，.gitignore 已屏蔽，无需手动清理。
归档注意事项：若使用 `--save`，Agent 的 LLM 推理可能异步完成（见 `[AGENT_PENDING]` 提示）。此时脚本会保存上下文到 `.llm-wiki/tmp/qmd-pending-query.json`，并提示使用 `--apply --slug <slug> --answer "<答案>"` 完成归档。强烈建议在 `--apply` 命令中保留原始问题文本作为位置参数。若遇到已存在的综述页，脚本会明确提示使用 `--update` 参数，Agent 应根据提示重新执行带 `--update` 的 `--apply` 命令。
...
检查项：孤儿页（豁免 sources/、syntheses/ 和 disambiguations/）、过时综述、缺失实体页（Raw 源文件链接已被排除）、稀疏页（仅统计 Wiki 内部出站双链，豁免 sources/、syntheses/ 和 disambiguations/）、Slug 冲突、图谱感知检查（可选）、加注缺失（LLM 辅助，检查所有来源目录的加注合规性）、知识盲区推荐（LLM 辅助）。
LLM 依赖项：加注缺失和知识盲区检查需要 Agent LLM 推理。首次运行时，lint 会将推理请求以 `[AGENT_LLM_REQUEST]` 形式输出并挂起（exit code 2）。Agent 完成推理后，应使用 `--apply-annotation-check` 或 `--apply-blind-spots` 参数回传结果，或将结果直接写入 `.llm-wiki/tmp/annotation-check-result.md` / `.llm-wiki/tmp/blind-spot-result.md` 文件，然后重新运行 lint 即可完成审计。缓存基于待检查词条集合的内容哈希自动刷新，确保条目变化时重新评估。
**退出码约定**同 §七。
## 九、知识图谱工作流（Graph Workflow）【可选扩展，50+ 词条后再启用】
触发：`build graph` → `python -m Tools.build_graph [选项]`

详细参数、两阶段机制、输出文件说明见 `python -m Tools.build_graph --help` 或脚本头部 docstring。
**唯一硬规则**：Agent 完成语义推断后必须用 `--apply-inferred` 合并结果，**禁止**直接重跑 `build_graph`，否则基础边和推断边都会重新生成，造成重复累积。需要重新推断时先 `--clean` 清除检查点。
## 十、命名规范与索引格式
...
## 十一、临时文件处理规范
所有临时文件统一存放于 `.llm-wiki/tmp/` 目录，.gitignore 已整体屏蔽，Agent 在 `git add` 前无需额外清理。脚本生成的关键临时文件：
- 语义检查上下文/回传：`.llm-wiki/tmp/semantic-lint-context.md`、`.llm-wiki/tmp/annotation-check-result.md`、`.llm-wiki/tmp/blind-spot-result.md`
- 图谱推断中间文件：`.llm-wiki/tmp/graph-infer-context.json`、`.llm-wiki/tmp/graph-infer-results.json`、`.llm-wiki/tmp/graph-cache.json`
...
- no-concept 审计链：批次验证失败触发 requeue 时若同时使用 --no-concept，log.md 不写入该豁免记录。队列状态正确更新，重跑不受影响。--no-concept 的传入时机与该文件在批次中的位置无关，任何一次 --finalize 调用中传入即生效；详见 §四 --no-concept 硬约束下的时机说明。
