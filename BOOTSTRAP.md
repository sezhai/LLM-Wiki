...
def scan_and_build_queue(candidates: List[str], mode: str = "normal") -> Optional[str]:
    ...
    # ── 按同文件名列聚簇的批次逻辑 ──
    # 将所有候选按 _series_grp 分组排序，同组内 vol 升序（前言=0 排前），
    # 然后按整个排序序列依次取前 DEFAULT_BATCH_SIZE 个。
    candidates.sort(key=lambda rel: _candidate_sort_key(REPO_ROOT / rel))
...
<!-- version: v1.0 | created: 2025-06-17 -->
...
1. 文件内容为纯摘录/引用堆砌，无作者自己的结构组织或概括提炼
2. 文件中不存在任何可提取的抽象概念（定理、机制、方法论、思想体系）
3. 文件中不存在任何可提取的唯一历史实体（人物、著作、机构、事件）

**以下情形严禁使用 `--no-concept`**（必须创建概念或实体词条）：

- ✅ 内容是对某部著作的结构化概括/摘要（如"冯友兰经典文存摘要"——虽然叫"摘要"，但按主题整理了新理学、新原人、新原道等体系性内容）
- ✅ 内容包含明确的术语定义或范畴体系（如"北溪字义摘要"——按命、性、心、情等范畴组织，本身就是概念词典）
- ✅ 内容围绕特定人物的思想体系展开（如"朱熹年谱摘要"——论学切要语部分系统阐述主敬穷理、格物致知等方法论概念）
- ✅ 内容系统阐述某一方法论或策略体系（如"穷查理宝典摘录"——多元思维模型、能力圈、逆向思维；"纳瓦尔宝典摘录"——三种杠杆、专长、复利）
- ✅ 内容为宗教/哲学经典的原文摘录（经文本身就是人类思想的核心文本，须创建对应 entity 如 [[圣经（基督教）]]、[[古兰经]]）
